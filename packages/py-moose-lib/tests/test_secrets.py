...
import pytest
from moose_lib.secrets import moose_env_secrets, get, MOOSE_ENV_SECRET_PREFIX


class TestMooseEnvSecretsGet:
    """Tests for the moose_env_secrets.get() method."""
    ...
    def test_creates_marker_with_correct_prefix(self):
        ...
        result = moose_env_secrets.get(var_name)

        assert result == f"{MOOSE_ENV_SECRET_PREFIX}{var_name}"
        assert result == "__MOOSE_ENV_SECRET__:AWS_ACCESS_KEY_ID"
    ...
    def test_can_be_used_in_s3queue_config(self):
        ...
        access_key_marker = moose_env_secrets.get("AWS_ACCESS_KEY_ID")
        secret_key_marker = moose_env_secrets.get("AWS_SECRET_ACCESS_KEY")
...
class TestModuleLevelGetFunction:
    ...
    def test_module_level_get_matches_class_method(self):
        ...
        result_class = moose_env_secrets.get(var_name)
...
class TestMooseEnvSecretPrefix:
    """Tests for the MOOSE_ENV_SECRET_PREFIX constant."""

    def test_has_expected_value(self):
        """Should have the expected prefix value."""
        assert MOOSE_ENV_SECRET_PREFIX == "__MOOSE_ENV_SECRET__:"

    def test_is_string(self):
        """Should be a string."""
        assert isinstance(MOOSE_ENV_SECRET_PREFIX, str)

    def test_is_not_empty(self):
        """Should not be empty."""
        assert len(MOOSE_ENV_SECRET_PREFIX) > 0
...
class TestMarkerFormatValidation:
    ...
    def test_creates_easily_detectable_markers(self):
        ...
        marker = moose_env_secrets.get("TEST_VAR")

        assert marker.startswith("__MOOSE_ENV_SECRET__:")
    ...
    def test_markers_can_be_split_to_extract_variable_name(self):
        ...
        var_name = "MY_SECRET"
        marker = moose_env_secrets.get(var_name)

        parts = marker.split(MOOSE_ENV_SECRET_PREFIX)
        assert len(parts) == 2
    ...
    def test_markers_work_with_dict_serialization(self):
        ...
        marker = moose_env_secrets.get("DATABASE_PASSWORD")
        ...
        assert MOOSE_ENV_SECRET_PREFIX in config["password"]
...
class TestIntegrationScenarios:
    ...
    def test_s3queue_engine_with_secrets(self):
        ...
        assert engine.aws_access_key_id == "__MOOSE_ENV_SECRET__:AWS_ACCESS_KEY_ID"
        assert engine.aws_secret_access_key == "__MOOSE_ENV_SECRET__:AWS_SECRET_ACCESS_KEY"

    def test_multiple_secrets_in_same_config(self):
        """Should handle multiple secrets in the same configuration."""
        config = {
            "username": moose_env_secrets.get("DB_USERNAME"),
            "password": moose_env_secrets.get("DB_PASSWORD"),
            "api_key": moose_env_secrets.get("API_KEY"),
        }

        # All should have the correct prefix
        for value in config.values():
            assert value.startswith(MOOSE_ENV_SECRET_PREFIX)
    ...
    def test_mixed_secret_and_plain_values(self):
        ...
        config = {
            ...
            "access_key": moose_env_secrets.get("AWS_ACCESS_KEY_ID"),  # Secret
            "bucket": "my-bucket",  # Plain value
            "secret_key": moose_env_secrets.get("AWS_SECRET_ACCESS_KEY"),  # Secret
        }
        ...
        assert MOOSE_ENV_SECRET_PREFIX in config["access_key"]
        assert MOOSE_ENV_SECRET_PREFIX in config["secret_key"]
