...
/// Handle LeaveGroup requests, removing members from consumer groups.
pub async fn handle(
    broker: &Broker,
    ...
    let group = match coordinator.groups.get_mut(&request.group_id) {
        Some(g) => g,
        None => {
            response.error_code = error::GROUP_ID_NOT_FOUND;
            return response;
        }
    };
    ...
    if api_version >= 3 {
        for member in &request.members {
            ...
            if group.members.contains_key(&member.member_id) {
                ...
                member_resp.error_code = error::UNKNOWN_MEMBER_ID;
            }
