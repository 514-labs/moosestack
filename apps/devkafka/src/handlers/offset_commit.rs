...
/// Handle OffsetCommit requests, storing committed offsets for consumer groups.
pub async fn handle(
    broker: &Broker,
    ...
    // Note: we intentionally skip group/member validation here. As a dev
    // broker we accept offset commits for any group, matching the behavior
    // of simple consumers that don't go through the JoinGroup protocol.
