...
impl Db {
    ...
    /// Returns Err on type mismatch (key holds a list).
    ...
    /// Notify the background expiry task that new expirations may have been added.
    ///
    /// Call this after mutating state directly (e.g. via `lock_state()` in Lua EVAL)
    /// so the purge task wakes up to handle newly-added TTLs.
    pub fn notify_expiry(&self) {
        self.shared.background_task.notify_one();
    }
    ...
    pub fn rpush(&self, key: String, values: Vec<Bytes>) -> Result<i64, String> {
        let mut state = self.shared.state.lock().unwrap();
        // Treat expired keys as non-existent so get_or_create_list creates fresh.
    ...
    pub fn rpoplpush(&self, source: &str, destination: &str) -> Result<Option<Bytes>, String> {
        ...
        // Treat expired destination as non-existent (consistent with rpush/lpop/lrem).
        if state.is_expired(destination) {
...
impl State {
    ...
    /// Returns Err on type mismatch (key holds a list).
    pub fn get_string(&self, key: &str) -> Result<Option<Bytes>, String> {
        match self.entries.get(key) {
            Some(entry) => {
                ...
                match &entry.data {
                    Value::String(val) => Ok(Some(val.clone())),
                    Value::List(_) => Err(
                        "WRONGTYPE Operation against a key holding the wrong kind of value".into(),
                    ),
                }
    ...
    /// Check if a key exists and is expired.
