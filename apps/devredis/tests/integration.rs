...
/// Bind a TcpListener on a free port and start the devredis server on it.
///
/// Returns (port, join_handle). By passing the already-bound listener to the
/// server, we avoid the TOCTOU race where the port could be taken between
/// finding it free and the server binding to it.
fn start_server_on_free_port() -> (u16, tokio::task::JoinHandle<()>) {
...
#[tokio::test]
async fn test_ping() {
    let (port, handle) = start_server_on_free_port();
