...
interface BootstrapOptions {
  ...
  grantManualBootstrapConsent?: boolean;
  requireAutomaticBootstrapConsent?: boolean;
  rerunAgentInit?: boolean;
  respectNoProjectsSkip?: boolean;
...
async function withManualBootstrapConsent<T>(
  context: vscode.ExtensionContext,
  workspaceRoot: string | null,
  run: () => Promise<T>,
): Promise<T> {
  return withRecordedAutomaticBootstrapConsent(
    workspaceRoot,
  ...
  await withActivity(
    ...
    async (activity) => {
      ...
      if (workspaceRoot && isPathInsideRoot(workspaceRoot, projectRoot)) {
        ...
        await withManualBootstrapConsent(context, workspaceRoot, async () =>
          configureCurrentWorkspace(
            context,
            state,
            outputChannel,
            true,
...
async function bootstrap(
  ...
  {
    activity,
    grantManualBootstrapConsent = false,
  ...
  try {
    ...
    if (activeProject) {
      ...
      const configureActiveWorkspace = async (): Promise<void> => {
        activity?.step("Checking Moose and 514 CLI availability...");
      ...
      if (grantManualBootstrapConsent) {
        await withManualBootstrapConsent(
          context,
          workspaceFolder.uri.fsPath,
          configureActiveWorkspace,
        );
      } else {
        await configureActiveWorkspace();
      }
    } else {
        ...
        async () => {
          ...
          await withActivity(
            ...
            async (activity) => {
              ...
              const workspaceRoot =
                state.activeProject ?
                  (getContainingWorkspaceFolder(state.activeProject)?.uri
                    .fsPath ?? null)
                : null;
              await withManualBootstrapConsent(
                context,
          ...
          await withActivity(
            ...
            async (activity) => {
              await bootstrap(context, state, outputChannel, {
                activity,
                grantManualBootstrapConsent: true,
                rerunAgentInit: true,
                respectNoProjectsSkip: false,
    ...
    vscode.workspace.onDidGrantWorkspaceTrust(() => {
      void withActivity(
        ...
        async (activity) => {
          ...
          await bootstrap(context, state, outputChannel, {
            activity,
            requireAutomaticBootstrapConsent: true,
            respectNoProjectsSkip: false,
          });
        },
      ).catch(() => undefined);
