...
const escapeGoatModulePromise = importModule<EscapeGoatModule>("escape-goat");

export function buildHarnessSplashHtml(
  nonce = crypto.randomUUID().replaceAll("-", ""),
): Promise<string> {
  return escapeGoatModulePromise.then(({ htmlEscape }) => {
...
export function showHarnessSplashPanel(
  ...
  void buildHarnessSplashHtml()
    ...
    .catch((error: unknown) => {
      ...
      panel.webview.html = `<!DOCTYPE html><html lang="en"><body>Failed to load the Fiveonefour harness guide.</body></html>`;
    });
