...
test("buildInstallerCommand installs both cli tools", () => {
  assert.equal(
    buildInstallerCommand(),
    "bash -i <(curl -sSfL https://fiveonefour.com/install.sh) moose,514",
  );
});
...
test("installLatestCli runs the installer and records detected versions", async () => {
  ...
  const versions = await installLatestCli(outputChannel, {
    ...
    async runProcess(
      ...
      processCalls.push({ args, command, options });
  ...
  assert.deepEqual(shellCommands, [buildInstallerCommand()]);
  assert.deepEqual(
    processCalls.map((call) => [call.command, call.args.join(" ")]),
    [
      ["514", "--version"],
      ["moose", "--version"],
    ],
