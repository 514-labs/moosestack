...
test("shouldSkipAutomaticBootstrapAfterLiveCheck clears stale no-project cache", async () => {
  const emptyWorkspaces = new Set(["/repo/a", "/repo/b"]);
  const clearedWorkspacePaths: string[] = [];

  const shouldSkip = await shouldSkipAutomaticBootstrapAfterLiveCheck(
    ["/repo/a", "/repo/b"],
    (workspaceFolderPath) => emptyWorkspaces.has(workspaceFolderPath),
    async (workspaceFolderPath) => workspaceFolderPath === "/repo/b",
    async (workspaceFolderPath) => {
      clearedWorkspacePaths.push(workspaceFolderPath);
    },
  );

  assert.equal(shouldSkip, false);
  assert.deepEqual(clearedWorkspacePaths, ["/repo/b"]);
});

test("shouldSkipAutomaticBootstrapAfterLiveCheck keeps skipping when no live config exists", async () => {
  const emptyWorkspaces = new Set(["/repo/a"]);
  const clearedWorkspacePaths: string[] = [];
  ...
  assert.deepEqual(clearedWorkspacePaths, []);
});
