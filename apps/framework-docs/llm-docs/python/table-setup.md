...
s3_events_table = OlapTable[S3Event](
    ...
    OlapConfig(
        ...
        engine=S3QueueEngine(
            ...
            # Use moose_env_secrets for runtime secret resolution (see below)
...
Instead, use `moose_env_secrets` to defer credential resolution until runtime:

```python
from moose_lib import OlapTable, OlapConfig, moose_env_secrets
...
**How it works:**
1. `moose_env_secrets.get("VAR_NAME")` creates a marker in your code
