...
For EXTERNALLY_MANAGED tables (especially when used as sources for Materialized Views), configure Moose to automatically create local copies:
...
**How it works with remote access:**
1. First run: Prompts for password showing host/user/database context
2. Stores password securely in system keychain
3. Creates local mirrors with sample data from production
4. Materialized Views can reference these tables with real data

**Fallback behavior when no remote connection is configured:**

If no remote connection is configured (no `[dev.remote_clickhouse]` and no `MOOSE_REMOTE_CLICKHOUSE_URL`), Moose automatically:
- Creates tables from your local code schema definitions (empty, no production data)
- Materialized Views still work because source tables exist
- Shows a clear warning about missing production data and schema drift potential
...
# KafkaEngine configuration (constructor parameters)
class KafkaEngine:
    def __init__(
        self,
        broker_list: str,   # Kafka broker addresses (e.g., "kafka:9092")
        topic_list: str,    # Topic name to consume from
        group_name: str,    # Consumer group name
        format: str,        # Data format (e.g., "JSONEachRow")
    ):
        pass

# Kafka-specific settings (go in OlapConfig.settings field)
kafka_settings = {
    ...
    "kafka_sasl_password": "pass",        # SASL password (use moose_runtime_env.get())
}
```

#### Kafka Engine Limitations

- **No ORDER BY**: Kafka is a streaming engine and doesn't support `order_by_fields`
