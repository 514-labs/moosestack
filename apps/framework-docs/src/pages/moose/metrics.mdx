...
Moose applications expose a health check endpoint at `/health` that monitors the status of all critical services. This endpoint is used by container orchestration systems like Kubernetes to determine the health of your application.

#### Health Check Response

The `/health` endpoint returns a JSON response with the status of each service:

```json
{
  "healthy": ["Redis", "ClickHouse", "Redpanda", "Consumption API"],
  "unhealthy": []
}
```
...
- **ClickHouse**: OLAP database (when OLAP features are enabled)
- **Redpanda**: Streaming platform (when streaming features are enabled)
- **Consumption API**: API server for consumption endpoints (when API features are enabled)

Each service is checked concurrently with a 2-second timeout. If a service is unavailable or times out, it will appear in the `unhealthy` array and the endpoint will return a 503 status.
...
The Consumption API also exposes an internal health endpoint at `/_moose_internal/health` used by the proxy for service monitoring. This endpoint is not intended for external use.
