...
app.get("/data", async (request, reply) => {
  ...
  const result = await moose.client.query.execute(sql`SELECT * FROM table LIMIT 10`);
...
- [Express](/moose/app-frameworks/typescript-express) - Most popular framework
...
- [TypeScript Frameworks](/moose/app-frameworks) - Compare all options
