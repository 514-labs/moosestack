...
Moose uses Docker to run ClickHouse, Redpanda, and Temporal locally. **Let's verify Docker is ready before creating your project.**
...
You should see a table with headers like `CONTAINER ID`, `IMAGE`, `COMMAND` (the table might be empty - that's fine).

<Callout type="warning" title="If you see an error" compact={true}>
...
**Wait 30-60 seconds** for Docker to fully start, then run `docker ps` again.

</Callout>

<Callout type="success" title="Checkpoint" icon={CheckCircle} compact={true}>
You should see the `docker ps` output without errors before proceeding.
</Callout>
...
## Step 3: Create Your Project (1 minute)
...
## Step 4: Understand Your Project (1 minute)
...
## Step 5: Test Your Pipeline (2 minutes)
...
## Step 6: Hot Reload Schema Changes (1 minute)
