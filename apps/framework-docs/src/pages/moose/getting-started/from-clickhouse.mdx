...
If Moose detects CDC‑managed tables (ClickPipes/PeerDB) during init, it will also generate an `external_models.py` and add `from external_models import *` to `app/main.py` so those external tables are loaded automatically.
</Python>
...
If Moose detects CDC‑managed tables (ClickPipes/PeerDB) during init, it will also generate an `externalModels.ts`and add `import "./externalModels"` to `app/index.ts` so those external tables are loaded automatically.
</TypeScript>
