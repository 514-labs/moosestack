...
Moose defaults to loading app code from the `app/` folder. Set `source_dir` in `moose.config.toml` to point at any directory (for example `src/` in a monorepo), and Moose will load your data models, APIs, and workflows from that location. The CLI also exposes the setting to runtimes through the `MOOSE_SOURCE_DIR` environment variable so both the TypeScript and Python executors import from the correct path.
...
# Available for custom tooling or CI
echo $MOOSE_SOURCE_DIR   # -> src
```

Updating this value allows you to reorganize projects without symlinks or wrapper scripts—Moose will scaffold, build, and deploy from the directory you specify.
