...
# Remote native protocol port (Default: 9000)
# native_port = 9440
...
# Note: native_port refers to ClickHouse's native TCP protocol port
#       - Use 9440 for TLS (secure)
#       - Use 9000 for non-TLS
#       This is different from HTTP ports (8443/8123) used in connection URLs
...
```bash
# Remote ClickHouse connection (complete URL with password)
# Alternative to [dev.remote_clickhouse] config file approach
# Note: URL uses HTTP/S port in the connection string (8443 for TLS, 8123 for non-TLS)
#       ClickHouse native port (9440/9000) is automatically inferred from the HTTP port
