...
Transforms can now return arrays, and Moose will emit each element as its own Kafka message. This is perfect for explode/unnest workloads where a single upstream record fan-outs into many downstream facts.

<TypeScript>
```ts filename="ArrayTransform.ts"
import { IngestPipeline, Stream, OlapTable } from "@514labs/moose-lib";
...
const orderEvents = new IngestPipeline<OrderEvent>("OrderEvent", { stream: true });
const orderItemTable = new OlapTable<OrderItem>("OrderItem", { orderByFields: ["orderId", "timestamp"] });
const orderItemStream = new Stream<OrderItem>("OrderItem", { destination: orderItemTable });

orderEvents.getStream().addTransform(orderItemStream, (event) =>
...
<Callout type="info">
Each object returned from the array becomes an individual message with its own offset and retry semantics. You no longer need manual producer loops to fan out data.
</Callout>
