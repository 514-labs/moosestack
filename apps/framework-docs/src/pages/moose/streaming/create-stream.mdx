...
Modify your stream's parallelisation/partitions. By default, this is set to 1. 

This can increase performance, at the cost of ordinality.
...
Modify your stream's retention period. By default, this is set to 7 days.

Increasing retention period increases data storage.

<TypeScript>
```typescript filename="StreamConfig.ts"
export const highThroughputStream = new Stream<Data>("high_throughput", {
  retentionPeriod: 86400       // (unit, seconds) Keep data for 1 day
});
```
</TypeScript>
...
high_throughput_stream = Stream[Data]("high_throughput", StreamConfig(
  retention_period=86400, # (unit, seconds) Keep data for 1 day
))
```
</Python>
