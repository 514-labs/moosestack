...
```bash
moose init <name> [TEMPLATE|LANGUAGE] [--location <location>] [--no-fail-already-exists]
...
- `[TEMPLATE|LANGUAGE]`: Optional template or language name (e.g., "python", "typescript", "python-empty", "typescript-empty"). If omitted, you'll be prompted to select a language.
...
- `--language <python|typescript>`: Programming language to use. **Required when using `--from-remote`**. If provided without a template, uses the default empty template for that language.

**Examples:**
```bash
# Initialize with Python (using language name as positional argument)
