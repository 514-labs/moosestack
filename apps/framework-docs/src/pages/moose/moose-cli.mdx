...
- `--timing`: Show elapsed time for operations, e.g., "completed in 234ms" or "completed in 2s 345ms" (default: false)
...
```
Planning completed in 145ms
Validation completed in 23ms
Execution completed in 1s 200ms
Persist State completed in 45ms
OpenAPI Gen completed in 89ms
```
