...
```bash
moose init <name> [TEMPLATE|LANGUAGE] [--location <location>] [--language <language>] [--no-fail-already-exists]
```
...
- `--language`: Programming language to use (e.g., "python" or "typescript"). If provided without a template, uses the default empty template for that language.
...
# Use --language flag instead
moose init --language python MyProject

# Specify a custom location
moose init MyProject python --location ./my-custom-path

# Initialize from a remote database
moose init MyProject --from-remote <CONNECTION_STRING>
```
