...
moose init PROJECT_NAME python --location ./my-custom-path

# Initialize from an existing ClickHouse database
moose init PROJECT_NAME --from-remote <CONNECTION_STRING> --language python

# Initialize from database with interactive connection setup
moose init PROJECT_NAME --from-remote --language typescript
```
