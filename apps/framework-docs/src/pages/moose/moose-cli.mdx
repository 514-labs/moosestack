...
- `--timing`: Show elapsed time for operations, e.g., "finished in 234ms" or "finished in 2.3s" (default: false)
...
```bash
moose seed clickhouse [--clickhouse-url <URL>] [--table <name>] [--limit <n> | --all] [--order-by <clause>] [--verify=true|false]
```
...
- `--verify`: Verify row counts after seeding (default: `true`). Counts are shown for the default database; use `moose query` for tables in other databases. Use `--verify=false` to skip.
