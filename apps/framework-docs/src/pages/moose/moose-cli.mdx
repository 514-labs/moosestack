...
```bash
moose seed clickhouse [--clickhouse-url <URL>] [--table <name>] [--limit <n> | --all] [--order-by <clause>] [--verify | --no-verify]
```
...
- `--verify` / `--no-verify`: Verify row counts after seeding by querying `system.tables` (default: `--verify`).
