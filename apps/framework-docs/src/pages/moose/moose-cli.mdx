...
```bash
moose init <name> [TEMPLATE] [--location <location>] [--no-fail-already-exists]
moose init <name> --from-remote [<CONNECTION_STRING>] --language <python|typescript> [--location <location>] [--no-fail-already-exists]
```
- `<name>`: Name of your app or service
- `[TEMPLATE]`: Optional template name (e.g., "python", "typescript", "python-empty", "typescript-empty", "python-fastapi"). Use `moose template list` to see all available templates. If omitted, you'll be prompted to select a language (which uses the empty template for that language).
- `--location` / `-l`: Optional location for your app or service (defaults to the project name)
- `--no-fail-already-exists`: Skip the directory existence check
- `--from-remote [<CONNECTION_STRING>]`: Initialize from an existing ClickHouse database. If connection string is omitted, you'll be prompted interactively.
- `--language <python|typescript>`: Programming language to use. **Required when using `--from-remote`**. Maps to the empty template for that language (e.g., `--language python` uses `python-empty`).

**Examples:**
```bash
# Initialize with Python template (bootstrapped with example components)
moose init MyProject python

# Initialize with TypeScript template (bootstrapped with example components)
moose init MyProject typescript

# Initialize with empty Python template
moose init MyProject python-empty

# Specify a custom location
moose init MyProject python --location ./my-custom-path

# Initialize from an existing ClickHouse database
moose init MyProject --from-remote <CONNECTION_STRING> --language python

# Initialize from database with interactive connection setup
moose init MyProject --from-remote --language typescript
```

**Alternative: Import from database after initialization**
```bash
# First initialize your project
moose init MyProject python

# Then use db pull to import from your database
moose db pull --clickhouse-url <CONNECTION_STRING>
```
...
```bash
moose seed clickhouse [--clickhouse-url <URL>] [--table <name>] [--limit <n> | --all]
```
- `--clickhouse-url`: Remote ClickHouse connection URL. If omitted, the CLI uses `MOOSE_SEED_CLICKHOUSE_URL`.
...
- `--type`: Filter by infrastructure type (tables, streams, ingestion, sql_resource, consumption, web-apps)
- `--name`: Filter by name
...
```
- `--clickhouse-url`: ClickHouse URL; native `clickhouse://` is auto-converted to HTTP(S). Include `?database=` or the CLI will query the current database.
