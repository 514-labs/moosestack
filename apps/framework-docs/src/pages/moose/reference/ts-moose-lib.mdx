...
export const dedupTable = OlapTable.withReplacingMergeTree<UserProfile>(
  "user_profiles",
  { 
    orderByFields: ["id", "timestamp"],
...
Configure S3Queue tables for streaming data from S3 buckets:
```ts
// Use the withS3Queue factory method
