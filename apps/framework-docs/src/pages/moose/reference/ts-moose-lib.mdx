...
Moose provides type aliases for temporal data that map to ClickHouse's DateTime types, 
giving you a choice between Date objects and strings based on your precision requirements.
...
A type for datetime values with configurable precision (0-9 decimal places).
JavaScript `Date` objects are used at runtime but cannot represent microsecond precision accurately.
...
**Caution:** JavaScript Date objects cannot accurately represent microsecond precision.
If you use `DateTime64<4>` or higher, microseconds will be lost during JSON parsing.

### `DateTimeString`
A string type with date-time format validation, preserved as a string at runtime.
```ts
interface Event {
...
A string type for high-precision datetime values,
preserving full precision as strings to avoid JavaScript Date limitations.
```ts
interface Event {
  // Stored as ClickHouse DateTime64(6) - microsecond precision
...
**Important:** This is the **only** way to preserve microsecond or higher precision.
Using `DateTime64<6>` or above without the String suffix will result in precision loss.
