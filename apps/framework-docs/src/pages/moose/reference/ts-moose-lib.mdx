...
```ts
import { getMaterializedViews } from "@514labs/moose-lib";

const mvs = getMaterializedViews();  // Returns Map<string, MaterializedView<any>>
for (const [name, mv] of mvs) {
  console.log(`MV: ${name}, target: ${mv.targetTable.name}`);
}
```
...
```ts
import { getMaterializedView } from "@514labs/moose-lib";

const mv = getMaterializedView("UserStatsMV");
if (mv) {
  console.log(`Target table: ${mv.targetTable.name}`);
}
```
...
```ts
import { getCustomViews } from "@514labs/moose-lib";

const views = getCustomViews();  // Returns Map<string, View>
...
### `getCustomView(name)`
Get a specific custom view by name.
```ts
import { getCustomView } from "@514labs/moose-lib";

const view = getCustomView("ActiveUsersView");
if (view) {
