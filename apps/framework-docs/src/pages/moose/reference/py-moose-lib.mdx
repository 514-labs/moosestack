...
```python
from moose_lib import get_materialized_views

mvs = get_materialized_views()  # Returns Dict[str, MaterializedView]
for name, mv in mvs.items():
    print(f"MV: {name}, target: {mv.target_table.name}")
```

### `get_materialized_view(name)`
Get a specific materialized view by name.
```python
from moose_lib import get_materialized_view

mv = get_materialized_view("UserStatsMV")
if mv:
    print(f"Target table: {mv.target_table.name}")
```

### `get_custom_views()`
Get all registered custom views.
```python
from moose_lib import get_custom_views

views = get_custom_views()  # Returns Dict[str, View]
for name, view in views.items():
    print(f"View: {name}")
```

### `get_custom_view(name)`
Get a specific custom view by name.
```python
from moose_lib import get_custom_view

view = get_custom_view("ActiveUsersView")
if view:
