...
<Callout type="info" title="New to migrations?" href="/moose/migrate" ctaLabel="Read the concept guide" compact>
...
If you're integrating Moose OLAP into an existing app, see [How to Use Planned Migrations with Library Model](/moose/olap/planned-migrations-library) instead.
...
<Callout type="info" title="Admin API required" href="/moose/apis/admin-api" ctaLabel="Admin API setup" compact>
To run `moose generate migration` against production, your Admin API must be enabled and authentication configured on your MooseStack runtime. Follow the setup guide: [Admin API setup](/moose/apis/admin-api).
</Callout>
...
If you see `DropTableColumn` operations, verify they're intentional. Moos Migrate doesn't distinguish column renames from drop-and-recreate, so a rename will appear as a drop followed by an add.
</Callout>
...
See the [migration reference](/moose/migrate/reference) for all available operations.
...
- Ensure [admin authentication is configured](/moose/apis/auth#admin-endpoints)
...
- See [troubleshooting schema changes](/moose/olap/schema-change) for data type compatibility issues

## Next Steps

- [How to use planned migrations with library model](/moose/getting-started/add-to-app) - For integrating into existing apps
- [Migration reference](/moose/migrate/reference) - Operations, data types, and YAML reference
