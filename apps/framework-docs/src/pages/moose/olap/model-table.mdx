...
export const dedupTable = OlapTable.withReplacingMergeTree<ExampleSchema>(
  "example_table",
  ["id", "dateField"],
  {
...
const dedupTable = OlapTable.withReplacingMergeTree<Record>(
  "table",
  ["id"],
  {
...
// Basic deduplication
const table = OlapTable.withReplacingMergeTree<Record>("table", ["id"]);

// With version column (keeps record with highest version)
const versionedTable = OlapTable.withReplacingMergeTree<Record>(
  "table",
  ["id"],
...
const softDeleteTable = OlapTable.withReplacingMergeTree<Record>(
  "table",
  ["id"],
  { ver: "updated_at", isDeleted: "deleted" },  // UInt8 column: 1 marks row for deletion
);
```
</TypeScript>
