...
import { OlapTable, Key, ClickHouseDefault, ClickHouseDecimal } from "@514labs/moose-lib";
...
interface Event {
  ...
  amount: (string & ClickHouseDecimal<10, 2> & ClickHouseDefault<"0">);
}
...
### Table engines
By default, Moose will create tables with the `MergeTree` engine. You can use different engines through factory methods or engine configuration classes.
...
// Use factory methods for other engines
const dedupTable = OlapTable.withReplacingMergeTree<Record>("table", {
  orderByFields: ["id"],
...
// Basic deduplication
const table = OlapTable.withReplacingMergeTree<Record>("table", {
  orderByFields: ["id"]
});

// With version column (keeps record with highest version)
const versionedTable = OlapTable.withReplacingMergeTree<Record>("table", {
  orderByFields: ["id"],
  ver: "updated_at"  // Column that determines which version to keep
});

// With soft deletes (requires ver parameter)
const softDeleteTable = OlapTable.withReplacingMergeTree<Record>("table", {
  orderByFields: ["id"],
  ver: "updated_at",
...
// Modern API using factory method
export const s3Events = OlapTable.withS3Queue<S3Event>(
  "s3_events",
