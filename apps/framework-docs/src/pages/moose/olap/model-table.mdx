...
Use the `ReplacingMergeTree` engine to keep only the latest record for your designated sort key. The default deduplication will keep the latest record & can be customized to use the maximum of a specific field.

<TypeScript>
```ts filename="DeduplicatedTable.ts" copy
import { Key, OlapTable, ClickHouseInt, ClickHouseEngines } from "@514labs/moose-lib";
...
const table = new OlapTable<Record>("table", {
  orderByFields: ["id"],
  engine: ClickHouseEngines.ReplacingMergeTree,
  dedupByField: "version" // optional, keep the max based on version field
});
```
</TypeScript> 
...
class Record(BaseModel):
    id: Key[str]
    version: Annotated[int, "uint64"]

table = OlapTable[Record]("table", OlapConfig(
  orderByFields=["id"],
  engine=ClickHouseEngines.ReplacingMergeTree,
  dedup_by_field="version" # optional, keep the max based on version field
))
```
</Python>
