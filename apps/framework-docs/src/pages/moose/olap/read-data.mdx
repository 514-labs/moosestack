...
def get_active_users(status: str, limit: int):
    ...
    query = """
        ...
        LIMIT {limit:UInt32}
    """
    return client.query.execute_raw(query, {"table": UserTable, "status": status, "limit": limit})
...
def build_validated_query(params: UserQueryParams):
    ...
    return client.query.execute_raw(query, {"table": UserTable, "status": params.status, "limit": params.limit})
```
</Tabs.Tab>
</Tabs>
