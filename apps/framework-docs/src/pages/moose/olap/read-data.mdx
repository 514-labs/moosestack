...
## Two Approaches for Python Queries

Moose provides two distinct approaches for executing queries in Python. Choose the right one for your use case:

### 1. execute_raw with Parameter Binding (Recommended)

**Use when:** Building dynamic queries, handling user input, or when you need type safety.
...
# Safe parameter binding with explicit ClickHouse types
query = """
  SELECT id, name, email
  FROM users
  WHERE status = {status:String}
...
### 2. execute with F-strings (Simple Cases Only)

**Use when:** Simple, static queries with trusted data that you control.

**Benefits:**
- Simpler syntax for basic cases
- No parameter dictionary needed

**WARNING:** Only use with trusted, validated input to prevent SQL injection.
...
**Security Guidelines:**
- **execute_raw**: Use for all dynamic queries and user input
- **execute with f-strings**: Only for simple, static queries with trusted data
- Always validate input with Pydantic when using f-strings
- Never use f-strings with unvalidated user input
