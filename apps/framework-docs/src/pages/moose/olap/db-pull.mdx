...
- **--clickhouse-url**: Required. ClickHouse URL (native or HTTP/S)
...
### New project from an existing DB
  If you're starting with an existing ClickHouse database, you can use `--from-remote` to bootstrap code, then use `db pull` over time to keep external models fresh:
  ```bash filename="Terminal" copy
  ...
  **Alternative: Two-step process**
  ...
  moose db pull --clickhouse-url $REMOTE_CLICKHOUSE_URL
  ```
