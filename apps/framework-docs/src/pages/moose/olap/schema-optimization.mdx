...
### Sample BY expressions

ClickHouse can scan representative subsets of data when you declare a `SAMPLE BY` expression. Moose now surfaces this option so you can keep fast dashboards and approximate analytics inside your table definitions.

<TypeScript>
```ts filename="SampleBy.ts"
import { OlapTable } from "@514labs/moose-lib";
...
export const userActivity = new OlapTable<UserActivity>("UserActivity", {
  orderByFields: ["timestamp", "userId"],
  sampleByExpression: "cityHash64(userId, sessionId)"
});
```
</TypeScript>

<Python>
```py filename="sample_by.py"
...
user_activity = OlapTable[UserActivity]("UserActivity", OlapConfig(
    order_by_fields=["timestamp", "user_id"],
    sample_by_expression="cityHash64(user_id, session_id)"
))
```
</Python>

Pick keys that distribute evenly (hashes of IDs, session tuples, etc.) so sampled queries stay accurate. You can still run full-table scans; `SAMPLE` is entirely opt-in per query.
