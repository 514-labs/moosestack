...
ClickHouse enums support both string values and integer values. Moose fully supports **Enum8** and **Enum16** with their complete ranges, including negative values.
...
interface ApiResponse {
  status: HttpStatus;    // Maps to Enum16 in ClickHouse
  level: ErrorLevel;     // Maps to Enum8 in ClickHouse
}
```
</TypeScript>
...
<Callout type="info" title="ClickHouse Enum Ranges">
Moose automatically selects the appropriate ClickHouse enum type based on your values:
- **Enum8**: Values from **-128 to 127** (1 byte)
- **Enum16**: Values from **-32,768 to 32,767** (2 bytes)

When using `moose db-pull` to import existing ClickHouse tables, enum columns are correctly detected and mapped to your application code with full support for negative values and the complete numeric range.
</Callout>
