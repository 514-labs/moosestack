...
`SimpleAggregateFunction` stores pre-aggregated values for use with `AggregatingMergeTree` tables. It's optimized for simple aggregation functions like `sum`, `min`, `max`, and `anyLast`.
