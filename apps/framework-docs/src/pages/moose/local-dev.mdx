...
The MCP server runs automatically at `http://localhost:4000/mcp`. Configure your AI client (Cursor, Claude Desktop, VS Code, Windsurf) to connect:
...
```json copy
{
  "mcpServers": {
    "moose-dev": {
      "command": "node",
      "args": ["-e", "const http = require('http'); const options = { hostname: 'localhost', port: 4000, path: '/mcp', method: 'POST', headers: { 'Content-Type': 'application/json' } }; process.stdin.pipe(http.request(options, (res) => { res.pipe(process.stdout); }));"]
    }
  }
}
```
...
- *"List all streaming topics in my project"*

<Callout type="info" title="Full MCP Documentation" href="/moose/mcp-server" ctaLabel="Learn more">
