...
### Use case: automatic mirroring of external tables

If you work with `EXTERNALLY_MANAGED` tables (e.g., from CDC services), you can configure Moose to automatically create local copies during development:
...
**Benefits:**
- ✅ **Materialized Views work** with external source tables
- ✅ **Test with real production data** (sample size configurable)
- ✅ **Automatic fallback** - Creates empty tables from code schema if no remote access
- ✅ **Secure** - Password stored in keychain, never in config files

**How it works:**
1. First run: Prompts for password (with context: host/user/database)
2. Stores password securely in system keychain
3. Creates local copies with sample data
4. Materialized Views can reference these tables

<Callout type="info" title="Learn more" ctaLink="/moose/olap/external-tables#development-mode-configuration">
See the [External Tables documentation](/moose/olap/external-tables#development-mode-configuration) for complete configuration options.
</Callout>
