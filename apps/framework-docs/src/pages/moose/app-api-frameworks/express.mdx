...
Protect your Express endpoints with API key authentication using `expressApiKeyAuthMiddleware()`.
...
This outputs:
- **ENV API Keys**: Hashed key for `MOOSE_WEB_APP_API_KEYS` (store on server)
...
export MOOSE_WEB_APP_API_KEYS="a1b2c3..."  # Use the hash from step 1
```

For multiple keys (key rotation):
```bash
export MOOSE_WEB_APP_API_KEYS="hash1,hash2,hash3"
```
...
```ts filename="app/apis/protectedApi.ts" copy
import express from "express";
import {
  WebApp,
  expressMiddleware,
...
app.use(express.json());
app.use(expressMiddleware()); // Required: adds Moose utilities
...
app.get("/protected/data", async (req, res) => {
  const moose = getMooseUtils(req);
  // This route requires valid API key
  const data = await moose.client.query.execute(/* ... */);
  res.json(data);
});

export const protectedApi = new WebApp("protected", app, {
  mountPath: "/protected",
});
```
...
app.get("/data", expressApiKeyAuthMiddleware(), async (req, res) => {
...
```bash
curl -H "Authorization: Bearer token.salt" \
  http://localhost:4000/protected/data
```

```ts
// JavaScript/TypeScript client
const response = await fetch("http://localhost:4000/protected/data", {
  headers: {
...
- **Uses PBKDF2 HMAC SHA256** (1000 iterations, 20-byte key) matching the Rust CLI implementation
- **Constant-time comparison** prevents timing attacks
- **Fail-secure design**: If `MOOSE_WEB_APP_API_KEYS` is not set, all requests are rejected (401)
- **Key rotation supported**: Comma-separate multiple hashes for zero-downtime rotation
- **Minimum key length**: 32 characters recommended (warning issued for shorter keys)
