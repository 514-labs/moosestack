...
Mount Express applications within your Moose project using the `WebApp` class. Express is the most popular Node.js web framework with a rich ecosystem of middleware.

<Callout type="info" title="Choose your integration path">
  - Already run Express elsewhere? Keep it outside Moose and query data with the Moose client. The [Querying Data guide](/moose/olap/read-data) shows how to use the SDK.  
  - Want Moose to host Express? Follow the steps below to mount it with `WebApp` and share Moose auth, logging, and deployment tooling.
</Callout>
...
- Avoid: `/api`, `/admin`, `/consumption`, `/health`, `/ingest`
- Use: `/myapi`, `/v1`, `/custom`
