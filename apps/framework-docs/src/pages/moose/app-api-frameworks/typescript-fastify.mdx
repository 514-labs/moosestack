...
Mount Fastify applications within your Moose project using the `WebApp` class. Fastify is a fast and low overhead web framework with powerful schema-based validation.

<Callout type="info" title="Choose your integration path">
  - Already running Fastify elsewhere? Keep it outside Moose and query data with the Moose client. The [Querying Data guide](/moose/olap/read-data) walks through the SDK.  
  - Want Moose to host Fastify? Follow the setup below to mount it with `WebApp` and share Moose auth, logging, and deployment tooling.
</Callout>
...
app.get("/data", async (request, reply) => {
  ...
  const { client, sql } = moose;
  const result = await client.query.execute(sql`SELECT * FROM ${MyTable} LIMIT 10`);
