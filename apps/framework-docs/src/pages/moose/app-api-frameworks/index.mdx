...
<Callout type="warning" title="Official Moose WebApp support">
  `WebApp` currently ships adapters for Express, Koa, Fastify, raw Node.js, and FastAPI. Frameworks like Next.js can call Moose today through the client pattern above, but they are not yet first-class `WebApp` integrations.
</Callout>

### TypeScript

<div className="grid grid-cols-1 md:grid-cols-2 gap-4 mt-6">
  <Link href="/moose/app-api-frameworks/typescript-native" className="rounded-xl border bg-card text-card-foreground shadow p-6 hover:bg-muted transition">
    <div className="font-semibold">Native Moose APIs</div>
...
<div className="grid grid-cols-1 md:grid-cols-2 gap-4 mt-6">
  <Link href="/moose/app-api-frameworks/python-native" className="rounded-xl border bg-card text-card-foreground shadow p-6 hover:bg-muted transition">
    <div className="font-semibold">Native Moose APIs</div>
    <div className="text-sm text-gray-600 dark:text-gray-400">
      Moose's built-in approach for simple GET endpoints
    </div>
