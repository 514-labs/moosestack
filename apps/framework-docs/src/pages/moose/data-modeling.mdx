...
### Python naming and Pydantic aliases

<Callout type="info" title="Invalid/underscore-prefixed field names in Python" compact={true}>
If a ClickHouse column name isn't a valid Python identifier or starts with an underscore, Moose generates a safe Python field name and sets a Pydantic alias to the real column name. Moose then uses the alias for ClickHouse DDL and data mapping, so your model remains valid while preserving the true column name.
