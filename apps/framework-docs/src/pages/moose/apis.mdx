...
def query_function(client: MooseClient, params: QueryParams) -> list[UserData]:
    ...
    query = "SELECT * FROM user_data WHERE user_id = {user_id:String} LIMIT {limit:UInt32}"
    return client.query.execute_raw(query, {"user_id": params.user_id, "limit": params.limit})
