...
import { Callout, LanguageSwitcher, TypeScript, Python, BulletPointsCard, CheckmarkBullets, ZoomImg, CompareBulletPointsCard, CTACard, CTACards } from "@/components";
...
## Validation
Moose validates all incoming data against your <TypeScript inline>interface</TypeScript><Python inline>Pydantic model</Python>. If a record fails validation, Moose can automatically route it to a Dead Letter Queue (DLQ) for later inspection and recovery.
