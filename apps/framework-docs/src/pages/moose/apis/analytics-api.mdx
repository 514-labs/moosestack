...
<Tabs.Tab>
Use parameter binding even when selecting simple columns to avoid injection:

```python filename="ExampleApiSafe.py" copy
from moose_lib import Api, MooseClient
from pydantic import BaseModel
...
def run(client: MooseClient, params: QueryParams) -> list[ResponseBody]:
    ...
    return client.query.execute_raw(query, {"table": SourceTable, "category": params.filterField, "limit": params.maxResults})
...
```python filename="ExampleApiFString.py" copy
from moose_lib import MooseClient

def run(client: MooseClient, params: QueryParams):
  # Using f-string for simple static query
  query = f"""
    SELECT * FROM UserTable
  """
...
You can use f-strings with Pydantic validation to build safe queries:

<Tabs items={["execute_raw (Recommended)", "f-strings (Use with Caution)"]}>
<Tabs.Tab>
Use `execute_raw` with parameter binding for maximum security:
...
```python filename="SafeQueriesFString.py" copy
from pydantic import BaseModel, Field
...
<Callout type="warning">
When using f-strings, you MUST be extremely careful with your Pydantic model definitions:
...
# WARNING: This approach requires strict validation
```

<Callout type="warning">
When using f-strings, validate every interpolated value strictly (patterns, bounds, enums) to avoid injection.
</Callout>
...
def run(client: MooseClient, params: QueryParams):
    # Use Identifier binding for column and table
    query = "SELECT {column: Identifier} FROM {table: Identifier}"
...
def run(client: MooseClient, params: QueryParams, jwt: dict):
    ...
    query = """SELECT * FROM userReports WHERE user_id = {user_id:UInt64} LIMIT 5"""
    return client.query.execute_raw(query, {"user_id": jwt["userId"]})
```
</Python>
...
def run(client: MooseClient, params: QueryParams):
    ...
    raw_results = client.query.execute_raw(query, {"category": params.category, "limit": params.maxResults})
