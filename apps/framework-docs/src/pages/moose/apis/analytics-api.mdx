...
# Define the route handler function (using f-strings for simple queries)
def run(client: MooseClient, params: QueryParams) -> list[ResponseBody]:
    # Simple f-string query (only for trusted, static data)
    category = params.filterField
...
def run(client: MooseClient, params: QueryParams):
    # Using f-string with validated column name
...
def run(client: MooseClient, params: QueryParams, jwt: dict):
    # Using f-string with JWT data
    query = f"""SELECT * FROM userReports WHERE user_id = {jwt["userId"]} LIMIT 5"""
    return client.query.execute(query)
```
</Python>
