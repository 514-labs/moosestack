...
Added a "Copy Page" button to documentation and guide pages that copies the current page content as clean markdown to the clipboard. This makes it easier to share documentation content or use it with AI assistants.

![Copy page as markdown button](/release-notes/copy-as-markdown.png)

PR: [#3170](https://github.com/514-labs/moosestack/pull/3170)
...
PR: [#2966](https://github.com/514-labs/moosestack/pull/2966)

### Add ON CLUSTER support for database creation in ClickHouse clusters

When using ClickHouse clusters, databases are now automatically created on all cluster nodes using ON CLUSTER clauses. This ensures that custom databases are properly replicated across all nodes in the cluster.

PR: [#3120](https://github.com/514-labs/moosestack/pull/3120)
