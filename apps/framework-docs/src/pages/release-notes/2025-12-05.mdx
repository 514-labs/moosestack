...
// Configure table with IcebergS3 engine for data lake storage
export const AnalyticsEventsTable = new OlapTable<AnalyticsEvent>(
  "AnalyticsEvents",
  {
    ...
    awsAccessKeyId: "{{ AWS_ACCESS_KEY_ID }}", // Environment variable
...
PR: [#2978](https://github.com/514-labs/moosestack/pull/2978)
...
// Table using primaryKeyExpression with hash function for better distribution
...
PR: [#3031](https://github.com/514-labs/moosestack/pull/3031)
...
# Define your data model using MooseModel for autocomplete support
...
# Build SQL queries with IDE autocomplete for column names
def create_user_analytics_query():
    # The {Column:col} format provides proper SQL quoting and autocomplete
    query = f"""
    SELECT
        {UserEvent.user_id:col},
        {UserEvent.event_type:col},
        COUNT(*) as event_count,
        SUM({UserEvent.revenue:col}) as total_revenue
    FROM user_events
    WHERE {UserEvent.timestamp:col} >= '2024-01-01'
        AND {UserEvent.session_id:col} IS NOT NULL
    GROUP BY {UserEvent.user_id:col}, {UserEvent.event_type:col}
    ORDER BY total_revenue DESC
    """
    return query
```

PR: [#3024](https://github.com/514-labs/moosestack/pull/3024)
...
- TypeScript tests template Zod import path fix – [#3038](https://github.com/514-labs/moosestack/pull/3038)
- SQL parsing for ClickHouse array literals in views – [#3034](https://github.com/514-labs/moosestack/pull/3034)
- LowCardinality(String) column type handling in peek/query commands – [#3025](https://github.com/514-labs/moosestack/pull/3025)
- DateTime precision loss – preserve microseconds/nanoseconds with new DateTime64String types – [#3018](https://github.com/514-labs/moosestack/pull/3018)
...
## Other Boreal improvements

- **Log drain performance** – Reuses database connections instead of creating new ones per batch, reducing connection overhead. PR [#1121](https://github.com/514-labs/commercial/pull/1121)
- **ClickHouse query performance** – Optimized table structure and indexing for faster loading of query performance data. PR [#1116](https://github.com/514-labs/commercial/pull/1116)
- **Temporarily removed "Build from Existing Database"** – Option removed from project creation flow while being improved. Users can still import from GitHub or templates. PR [#1104](https://github.com/514-labs/commercial/pull/1104)

---

## Infrastructure improvements

- **Extended deployment startup timeouts** – Increased from 60 to 180 seconds to reduce failures for larger applications during high-load periods. PR [#1098](https://github.com/514-labs/commercial/pull/1098)
