...
* **Breaking:** Update HubSpot connector configuration parameters
...
### ⚠️ Breaking Changes
- **Update HubSpot connector configuration parameters**
  Updated HubSpot connector to use 'ingestApi' parameter instead of 'ingest' and changed environment variable from HUBSPOT_ACCESS_TOKEN to HUBSPOT_TOKEN. Users will need to update their configuration and environment variables when upgrading.

  **Updated IngestPipeline with ingestApi parameter**
  ```typescript
  import { IngestPipeline, OlapTable, ClickHouseEngines } from "@514labs/moose-lib";
  ...
  const customerDeadLetterTable = new OlapTable<any>("CustomerDeadLetter", {
    engine: ClickHouseEngines.MergeTree,
    orderByFields: ["timestamp"]
  });
  
  // Updated configuration: use 'ingestApi' instead of 'ingest'
  export const CustomerPipeline = new IngestPipeline<CustomerData>("Customer", {
    table: {
      engine: ClickHouseEngines.MergeTree, 
      orderByFields: ["id"]
    },
    stream: true,
    ingestApi: true, // ✅ New parameter name
    deadLetterQueue: { 
      destination: customerDeadLetterTable 
    },
  });
  
  // For derived/processed data pipelines, set ingestApi to false
  export const ProcessedCustomerPipeline = new IngestPipeline<CustomerData>("ProcessedCustomer", {
    table: {
      engine: ClickHouseEngines.MergeTree,
      orderByFields: ["id"]
    },
    stream: true,
    ingestApi: false, // No direct API endpoint needed
  });
  ```
  
  Shows how to update IngestPipeline configurations to use the new 'ingestApi' parameter instead of the deprecated 'ingest' parameter. The example demonstrates both enabling and disabling the ingest API for different pipeline types.
...
- **Add support for ignoring partition changes during migrations**
  ...
  **Configure migrations to ignore partition changes**
  ```typescript
  // moose.config.ts - Configure migration to ignore partition changes
  ...
  export default {
    ...
    migration: {
      ...
      ignoreOperations: ['ModifyPartitionBy']
    },
    
    // Production environment settings
    production: {
      // Migration config is only applied in production mode
      migration: {
        ignoreOperations: [
          'ModifyPartitionBy',  // Ignore partition changes
          // Add other operations to ignore as needed
        ]
      }
    }
  } satisfies MooseConfig;
  
  // models/UserEvents.ts - Data model with partition configuration
  ...
  // The partition configuration can be changed without triggering
  // table recreation when ModifyPartitionBy is in ignoreOperations
  export const UserEventsTable = {
    partitionBy: 'toYYYYMM(timestamp)', // This can be modified safely
    orderBy: ['userId', 'timestamp']
  };
  ```
  
  This example shows how to configure MooseStack to ignore partition changes during migrations. When ModifyPartitionBy is added to ignoreOperations, changing the partition configuration won't trigger expensive table recreations, reducing deployment downtime.

- **New documentation site with language-specific docs and AI integration**
  Launched a completely redesigned documentation site (framework-docs-v2) with separate TypeScript and Python documentation paths, full-text search, AI assistant integration via llms.txt generation, and improved navigation. The new site provides better developer experience with language-specific examples and enhanced search capabilities.
  
  📸 *[Screenshot needed: Launched a completely redesigned documentation site (framework-docs-v2) with separate TypeScript and Python documentation paths, full-text search, AI assistant integration via llms.txt generation, and improved navigation. The new site provides better developer experience with language-specific examples and enhanced search capabilities.]*
...
- **Add ClickHouse cluster configuration support for ON CLUSTER operations**
  ...
  **Configure ClickHouse clusters for distributed operations**
  ```typescript
  // moose.config.ts - Configure ClickHouse clusters for distributed operations
  import { MooseConfig } from "@514labs/moose-lib";
  ...
  // models/UserEvent.ts - Data model that will use clustered tables
  ...
  This example shows how to configure ClickHouse clusters in a MooseStack project to enable distributed table operations. The clusters are defined in the config file and then used with ON CLUSTER clauses in SQL to create tables and views across multiple ClickHouse nodes. In development mode, these clusters automatically work as single-node setups.
...
- **Add comprehensive connector development guide and GitHub agent**
  Added a detailed connector implementation guide that walks developers through the complete process of creating connectors for the Registry, from API research to production deployment. The guide includes code examples, authentication patterns, pagination strategies, and testing approaches. Also includes a GitHub agent to provide automated guidance during connector development.
  ...
  **Building a Shopify connector for MooseStack data ingestion**
  ...
  export async function ingestShopifyOrders() {
    ...
    console.log(`✅ Ingested ${totalOrders} orders from Shopify`)
  }
...
- **Add branch delete button in branch settings**
  ...
  📸 *[Screenshot needed: Added a delete button to the branch settings section, allowing users to delete branches directly from the settings page. Also improved loading states for environment variables pages and fixed commit message display formatting.]*
...
- **Added branch settings page with branch removal capability**
  ...
  📸 *[Screenshot needed: Added a new General settings tab for branches that allows users to remove non-default branches. Default branches are protected and cannot be removed. Also improved commit message display formatting to preserve line breaks across deployment views.]*
