...
- **Add support for custom TypeScript web frameworks in consumption APIs**
  ...
  **Express WebApp with MooseStack data integration**
  ```typescript
  ...
  app.get('/query', async (req, res) => {
    try {
      ...
      // Query your MooseStack data models
  ...
  app.post('/analytics', async (req, res) => {
    ...
    // Process analytics request using your data pipeline
    const results = await processAnalytics(startDate, endDate, metrics);
    
    res.json({ success: true, results, params: req.body });
  });
  ...
  Shows how to create a custom Express.js web application that integrates with MooseStack data pipelines. The WebApp is mounted at a custom path and can query ClickHouse data alongside standard Express routing and middleware.
...
- **Add support for Drizzle ORM type inference in TypeScript data models**
  Users can now define data models using Drizzle ORM's type inference capabilities. This allows for more flexible type definitions where the TypeScript type is inferred from a Drizzle schema rather than explicitly declared, enabling better integration with existing Drizzle-based codebases.

  **Using Drizzle ORM type inference in MooseStack data models**
  ```typescript
  import { boolean, pgTable, text } from "drizzle-orm/pg-core";
  import { ClickHouseEngines, OlapTable } from "@514labs/moose-lib";
  
  // Define your Drizzle schema as usual
  ...
  // Use Drizzle's type inference instead of declaring a separate interface
  type UserCredentials = typeof userCredentialsSchema.$inferSelect;
  
  // Create a MooseStack data model using the inferred type
  export const UserCredentials = new OlapTable<UserCredentials>(
    "UserCredentials",
    {
  ...
  // The inferred type automatically includes:
  // - userId: string
  // - isActive: boolean  
  // - email: string
  // No need to manually declare these types!
  ```
  
  This example shows how to use Drizzle ORM's type inference with MooseStack data models. Instead of manually declaring TypeScript interfaces, you can define a Drizzle schema and use `$inferSelect` to automatically generate the TypeScript type for your OlapTable.
...
- **Add support for geometry data types (Point, Polygon, LineString, etc.)**
  ...
  **Using geometry types in MooseStack data models**
  ```typescript
  import { Key } from "@514labs/moose-lib";
  ...
  const locationEvent: LocationEvent = {
    ...
    userLocation: [37.7749, -122.4194], // San Francisco
    
    // Delivery route with multiple waypoints
    deliveryRoute: [
      [37.7749, -122.4194], // Start
      [37.7849, -122.4094], // Waypoint
      [37.7949, -122.3994]  // End
    ],
    ...
    serviceArea: [[
      ...
      [37.8, -122.4],
      [37.7, -122.4],
      [37.7, -122.5] // Close the polygon
    ]],
    
    // Multiple delivery zones
    deliveryZones: [
      [[[37.7, -122.5], [37.75, -122.5], [37.75, -122.45], [37.7, -122.45], [37.7, -122.5]]],
      [[[37.8, -122.4], [37.85, -122.4], [37.85, -122.35], [37.8, -122.35], [37.8, -122.4]]]
    ],
    
    // Circular geofence around a point
    geofence: [
      [37.7749, -122.4194],
      [37.7759, -122.4184],
      [37.7739, -122.4184]
    ...
    alternativeRoutes: [
      [[37.7749, -122.4194], [37.7849, -122.4094]], // Route 1
      [[37.7749, -122.4194], [37.7849, -122.4294]]  // Route 2
    ]
  };
  ```
  
  This example shows how to define a data model using MooseStack's new geometry types for spatial data analysis. The LocationEvent interface demonstrates all supported geometry types (Point, LineString, Polygon, MultiPolygon, Ring, MultiLineString) in a realistic location-based analytics scenario.
...
- **Add scoped environment variables for organizations, projects, and branches**
  Environment variables can now be configured at three different levels: organization-wide (shared across all projects), project-specific (shared across all branches), or branch-specific. This provides more flexible configuration management and allows users to set common variables once at higher levels while overriding them for specific contexts when needed.
  
  📸 *[Screenshot needed: Environment variables can now be configured at three different levels: organization-wide (shared across all projects), project-specific (shared across all branches), or branch-specific. This provides more flexible configuration management and allows users to set common variables once at higher levels while overriding them for specific contexts when needed.]*
...
- **Added connection string options for database credentials**
  ...
  📸 *[Screenshot needed: Users can now select from multiple connection string formats (HTTPS URL, JDBC, Python Client, Node.js Client) when viewing database credentials, making it easier to connect to databases from different environments and programming languages.]*
...
- **Added new Branch Overview page with recent deployments**
  ...
  📸 *[Screenshot needed: Introduced a new Overview page for branches that displays recent deployments in an easy-to-view format. This replaces the previous default metrics page and provides users with a quick summary of their branch activity and deployment history.]*

### Improvements
- Release notes menu now displays newest releases first: The release notes sidebar menu now orders entries chronologically with the newest releases at the top, making it easier to find the latest updates and changes.
