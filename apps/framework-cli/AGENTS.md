...
- **Final pass**: prefer top-of-file `use` statements over inline ones in function bodies. Add `use` imports for fully-qualified paths. Existing inline `use` statements don't need to be moved unless you're already editing that code. Local `use` for disambiguation (e.g. name collisions) is fine.
