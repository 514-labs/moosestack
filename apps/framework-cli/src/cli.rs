...
use crate::framework::core::plan_risk::{
    classify_operations_risk, migration_destructive_gate, print_migration_rejected_guidance,
    ...
    match commands {
        ...
        Commands::Generate(generate) => match &generate.command {
            ...
            Some(GenerateCommand::Migration {
                ...
                let risk = classify_operations_risk(&result.db_migration.operations);
                ...
                if *no_auto_backfill_sql {
                    ...
                    let candidates = result.db_migration.detect_backfill_candidates(
                    ...
                    for check in &candidates {
                        match check {
                            BackfillCheckResult::Candidate(c) => {
                                ...
                                if should_append {
                                    result.db_migration.append_backfill(c);
                                    display::show_message_wrapper(
                                        MessageType::Success,
                                        Message {
