...
/// Global storage for embedded server handles so they can be shut down from anywhere.
...
///
/// No Docker dependency. devkafka and devredis run as embedded tokio tasks.
...
impl InfraProvider for NativeInfraProvider {
    ...
    fn stop(&self, project: &Project, _settings: &Settings) -> Result<(), RoutineFailure> {
        // Shut down embedded devkafka/devredis
