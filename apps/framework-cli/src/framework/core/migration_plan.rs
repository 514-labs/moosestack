...
impl MigrationPlan {
    ...
    pub fn from_infra_plan(
        ...
        .into_iter()
        .filter(|op| !Self::should_ignore_operation(op, ignore_ops, &partition_changed_tables))
        .collect();
        ...
        ignore_ops.iter().any(|ignore_op| {
            ...
            if matches!(ignore_op, IgnorableOperation::ModifyPartitionBy) {
                let table_name = match operation {
                    SerializableOlapOperation::DropTable { table, .. } => Some(table.as_str()),
                    SerializableOlapOperation::CreateTable { table } => Some(table.name.as_str()),
                    _ => None,
                };
                return table_name.is_some_and(|name| partition_changed_tables.contains(name));
            }
