...
/// # Serialization note
///
/// `External` uses a named `source` field rather than a newtype variant to avoid
/// duplicate `"type"` keys when serializing to JSON. This makes the JSON
/// TypeScript/Python-friendly: `{"type":"EXTERNAL","source":{"type":"HTTP",...}}`
/// rather than the duplicate-key form `{"type":"EXTERNAL","type":"HTTP",...}` that
/// standard JSON parsers (JavaScript, Python) cannot produce.
#[derive(Debug, Clone, PartialEq, Serialize, Deserialize)]
#[serde(tag = "type", rename_all = "SCREAMING_SNAKE_CASE")]
