...
pub fn extract_projections_from_create_table(sql: &str) -> Vec<ParsedProjection> {
        ...
        .filter(|s| s.to_uppercase().starts_with("PROJECTION "))
    {
        ...
        let projection_body = match body_end {
            ...
            // Uses quote-aware normalization to preserve whitespace inside
            // string literals (e.g., in WHERE clauses with string constants).
            Some(end) => normalize_whitespace_outside_quotes(&after_name[1..end]),
            None => continue,
        };
...
/// Normalizes whitespace outside of single-quoted string literals.
///
/// Collapses runs of whitespace (spaces, tabs, newlines) to a single space
/// outside quoted regions, while preserving whitespace inside quotes exactly
/// as-is. Uses [`quoted_ranges`] to identify quoted regions.
fn normalize_whitespace_outside_quotes(text: &str) -> String {
    let ranges = quoted_ranges(text);
    let mut result = String::with_capacity(text.len());
    let mut pos = 0;

    for range in &ranges {
        // Process unquoted region before this quoted range
        if pos < range.start {
            let unquoted = &text[pos..range.start];
            ...
            // If the unquoted region ends with whitespace and there's a quoted
            // region following, ensure a single space separator.
            if unquoted.ends_with(char::is_whitespace)
                && !result.is_empty()
                && !result.ends_with(' ')
            {
                result.push(' ');
        ...
        // Append quoted region verbatim
        result.push_str(&text[range.start..range.end]);
        pos = range.end;
    }

    // Process any remaining unquoted text after the last quoted range
    if pos < text.len() {
        let unquoted = &text[pos..];
        for token in unquoted.split_whitespace() {
            if !result.is_empty() && !result.ends_with(' ') {
                result.push(' ');
            }
...
SETTINGS enable_mixed_granularity_parts = 1, index_granularity = 8192"#;
    ...
    #[test]
    fn test_normalize_whitespace_outside_quotes_basic() {
        assert_eq!(
            normalize_whitespace_outside_quotes("SELECT  *   ORDER BY  id"),
            "SELECT * ORDER BY id"
        );
    }

    #[test]
    fn test_normalize_whitespace_outside_quotes_preserves_quoted_strings() {
        // Whitespace inside single-quoted strings must be preserved
        let input = "SELECT * WHERE name = 'hello  world'  ORDER BY id";
        let result = normalize_whitespace_outside_quotes(input);
        assert_eq!(result, "SELECT * WHERE name = 'hello  world' ORDER BY id");
    }

    #[test]
    fn test_normalize_whitespace_outside_quotes_multiple_quoted() {
    ...
    #[test]
    fn test_normalize_whitespace_outside_quotes_no_quotes() {
        let input = "  SELECT   _part_offset\n    ORDER BY   user_id  ";
        let result = normalize_whitespace_outside_quotes(input);
        assert_eq!(result, "SELECT _part_offset ORDER BY user_id");
    }
...
ORDER BY id"#;
        ...
        assert_eq!(
            projections[0].body, "SELECT _part_offset WHERE status = 'hello  world' ORDER BY id",
            "Whitespace inside quoted string should be preserved"
        );
    }
}
