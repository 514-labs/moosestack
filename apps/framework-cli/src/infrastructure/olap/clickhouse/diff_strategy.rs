...
pub fn nested_are_equivalent(
    ...
    for (actual_col, target_col) in actual.columns.iter().zip(target.columns.iter()) {
        ...
        let normalized_actual = normalize_column_for_low_cardinality_ignore(actual_col, ignore_low_cardinality);
        let normalized_target = normalize_column_for_low_cardinality_ignore(target_col, ignore_low_cardinality);
...
mod tests {
    ...
    fn test_nested_are_equivalent_with_ignore_low_cardinality() {
        ...
        assert!(!nested_are_equivalent(&nested_with_low_card, &nested_without_low_card, false));

        // When ignore_low_cardinality=true, they should be equivalent (this was the bug)
        assert!(nested_are_equivalent(&nested_with_low_card, &nested_without_low_card, true));
    }
}
