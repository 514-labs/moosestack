...
const DEFAULT_INSTALL_LSP: bool = true;
...
/// Runs `moose harness init`, including project scaffolding and user-level harness setup.
...
fn has_cli_inputs(flags: &HarnessInitArgs) -> bool {
    flags.name_option.is_some()
        || flags.template_option.is_some()
        || flags.name.is_some()
...
fn resolve_non_interactive_options(
    ...
    let from_remote = flags.from_remote.clone().or(request_from_remote);
...
async fn resolve_interactive_options(
    ...
    let install_lsp = prompt_bool("Install MooseStack LSP?", DEFAULT_INSTALL_LSP)?;
...
async fn install_harness_support(
    ...
    let installed_skills = skills
        ...
        .collect::<Vec<_>>();
    let http_client = skills
        .iter()
        .flat_map(|skill| skill.files.iter())
        .any(|file| matches!(&file.source, SkillFileSource::GitHubPath(_)))
        .then(build_http_client)
        .transpose()?;

    for skill in &skills {
        install_skill(skill, agents, home, branch, http_client.as_ref()).await?;
    }
...
async fn maybe_install_lsp(install_lsp_requested: bool) -> Result<bool, RoutineFailure> {
    if !install_lsp_requested {
        return Ok(false);
    }
...
fn build_http_client() -> Result<reqwest::Client, RoutineFailure> {
...
async fn fetch_repo_tree(
    ...
    let response = client
        .get(github_skill_tree_url(branch)?)
...
fn discover_skills_from_tree(tree: &GitTreeResponse) -> Result<Vec<SkillInfo>, RoutineFailure> {
    let skill_dirs = tree
        ...
        .collect::<Vec<_>>();
    let skill_prefixes = skill_dirs
        .iter()
        .map(|(repo_dir, _)| format!("{repo_dir}/"))
        .collect::<Vec<_>>();
    ...
    for (repo_dir, name) in skill_dirs {
        ...
        let repo_prefix = format!("{repo_dir}/");
        let nested_skill_prefixes = skill_prefixes
            .iter()
            .filter(|prefix| {
                prefix.starts_with(&repo_prefix) && prefix.as_str() != repo_prefix.as_str()
            })
            .cloned()
            .collect::<Vec<_>>();
        ...
        let files = tree
            ...
            .filter(|entry| {
                entry.entry_type == "blob"
                    && entry.path.starts_with(&repo_prefix)
                    && !nested_skill_prefixes
                        .iter()
            ...
            .map(|entry| {
                let relative_path = entry
                    ...
                    .expect("skill file path uses the repo prefix");
...
async fn install_skill(
    ...
    let staging_dir = tempfile::Builder::new()
        .prefix(&format!("{}-", flatten_skill_name(&skill.name)))
    ...
    for file in &skill.files {
        let destination = staging_path.join(&file.relative_path);
    ...
    remove_path_entry(&canonical_dir)?;
...
async fn download_raw_file(
    ...
    let response = client
        .get(github_raw_file_url(branch, path)?)
        .send()
        .await
        .map_err(|e| {
            RoutineFailure::error(Message::new(
...
fn valid_agent_schema_values() -> Vec<&'static str> {
    let mut values = vec!["auto", "none"];
    values.extend(valid_agent_ids());
    values
}
    ...
    let schema = serde_json::json!({
        ...
        "fields": {
            ...
            "agents": {
                ...
                "items": {
                    "type": "string",
                    "enum": valid_agent_schema_values(),
                }
            },
            "install_lsp": {
                "type": "boolean",
                "required": false,
                "default": DEFAULT_INSTALL_LSP
            },
            "branch": {
                "type": "string",
                "required": false,
...
mod tests {
    ...
    fn validate_relative_path_rejects_null_bytes() {
        let error = validate_relative_path("bad\0name").unwrap_err();
        assert!(error.message.details.contains("Unsafe skill path"));
    }

    #[test]
    fn validate_relative_path_rejects_parent_traversal() {
        for path in ["..", "../secret.txt", "dir/../etc"] {
            let error = validate_relative_path(path).unwrap_err();
            assert!(
                error.message.details.contains("Unsafe skill path"),
    ...
    #[test]
    fn discover_skills_from_tree_excludes_nested_child_skill_files() {
        let tree = GitTreeResponse {
            tree: vec![
                ...
                GitTreeEntry {
                    path: "skills/a/README.md".to_string(),
                    entry_type: "blob".to_string(),
                },
                GitTreeEntry {
                    path: "skills/a/b/SKILL.md".to_string(),
                    entry_type: "blob".to_string(),
                },
                GitTreeEntry {
                    path: "skills/a/b/examples/example.md".to_string(),
                    entry_type: "blob".to_string(),
                },
            ],
        };
    ...
    #[test]
    fn discover_local_skills_includes_nested_skills_without_child_files() {
        let root = tempfile::tempdir().unwrap();
        let skills_root = root.path().join("skills");
        let parent_dir = skills_root.join("a");
        let child_dir = parent_dir.join("b");

        std::fs::create_dir_all(parent_dir.join("docs")).unwrap();
        std::fs::create_dir_all(child_dir.join("examples")).unwrap();

        std::fs::write(parent_dir.join("SKILL.md"), "# Parent\n").unwrap();
        std::fs::write(parent_dir.join("README.md"), "parent docs\n").unwrap();
        std::fs::write(parent_dir.join("docs/guide.md"), "guide\n").unwrap();
        std::fs::write(child_dir.join("SKILL.md"), "# Child\n").unwrap();
        std::fs::write(child_dir.join("examples/example.md"), "example\n").unwrap();

        let skills = discover_local_skills(root.path()).unwrap();
        let parent_skill = skills.iter().find(|skill| skill.name == "a").unwrap();
        let child_skill = skills.iter().find(|skill| skill.name == "a/b").unwrap();
        ...
        let mut parent_files = parent_skill
            ...
            .collect::<Vec<_>>();
        parent_files.sort();
        assert_eq!(parent_files, vec!["README.md", "SKILL.md", "docs/guide.md"]);
    ...
    #[test]
    fn github_skill_tree_url_encodes_slash_containing_branch_names() {
    ...
    #[test]
    fn github_raw_file_url_encodes_slash_containing_branch_names() {
    ...
    #[test]
    fn valid_agent_schema_values_include_special_selectors() {
        let values = valid_agent_schema_values();
    ...
    #[test]
    fn parse_init_request_rejects_unknown_fields() {
        let error = parse_init_request(
            r#"{
                "version": 1,
                "name": "my-app",
                "template": "typescript",
        ...
        assert!(error.message.details.contains("unknown field"));
        assert!(error.message.details.contains("install_lps"));
    }

    #[test]
    fn resolve_non_interactive_options_defaults_to_installing_lsp() {
        let flags = HarnessInitArgs {
            action: None,
            name_option: None,
            template_option: None,
            name: Some("my-app".to_string()),
            template: Some("typescript".to_string()),
            location: None,
            no_fail_already_exists: false,
            from_remote: None,
            custom_dockerfile: false,
        ...
        let resolved = resolve_non_interactive_options(&flags, None).unwrap();
        assert!(resolved.install_lsp);
    }

    #[test]
    fn install_skill_replaces_stale_files_when_reinstalling() {
        let home = tempfile::tempdir().unwrap();
        let v1 = tempfile::tempdir().unwrap();
        let v2 = tempfile::tempdir().unwrap();

        let v1_skill = v1.path().join("SKILL.md");
        let v1_old = v1.path().join("old.md");
        std::fs::write(&v1_skill, "# v1\n").unwrap();
        std::fs::write(&v1_old, "old\n").unwrap();

        let v2_skill = v2.path().join("SKILL.md");
        let v2_new = v2.path().join("new.md");
        std::fs::write(&v2_skill, "# v2\n").unwrap();
        std::fs::write(&v2_new, "new\n").unwrap();

        let runtime = tokio::runtime::Runtime::new().unwrap();
        runtime
            .block_on(install_skill(
                &SkillInfo {
                    name: "clickhouse/best-practices".to_string(),
                    files: vec![
                        SkillFileSpec {
                            relative_path: "SKILL.md".to_string(),
                            source: SkillFileSource::LocalPath(v1_skill),
                        },
                        SkillFileSpec {
                            relative_path: "old.md".to_string(),
                            source: SkillFileSource::LocalPath(v1_old),
                        },
                    ],
                },
                &[],
        ...
        let canonical_dir = canonical_skill_dir(home.path(), "clickhouse/best-practices");
        assert!(canonical_dir.join("old.md").exists());
            ...
            .block_on(install_skill(
                &SkillInfo {
                    ...
                    files: vec![
                        SkillFileSpec {
                            relative_path: "SKILL.md".to_string(),
                            source: SkillFileSource::LocalPath(v2_skill),
                        },
                        SkillFileSpec {
                            relative_path: "new.md".to_string(),
                            source: SkillFileSource::LocalPath(v2_new),
                        },
                    ],
                },
                &[],
        ...
        assert!(canonical_dir.join("new.md").exists());
        assert!(!canonical_dir.join("old.md").exists());
    }

    #[test]
    #[serial_test::serial(harness_support)]
    fn install_harness_support_installs_lsp_without_agents() {
        let home = tempfile::tempdir().unwrap();
        ...
        let _path_guard = EnvVarGuard::set("PATH", std::env::join_paths(paths).unwrap());

        let runtime = tokio::runtime::Runtime::new().unwrap();
        let outcome = runtime
            .block_on(install_harness_support(
                home.path(),
                DEFAULT_SKILLS_BRANCH,
                &[],
                true,
        ...
        assert!(outcome.install_lsp_requested);
        assert!(outcome.lsp_installed);
        assert!(outcome.selected_agents.is_empty());
        assert!(outcome.installed_skills.is_empty());
        assert!(outcome.configured_mcp_agents.is_empty());
        assert!(outcome.configured_lsp_agents.is_empty());
        assert_eq!(
            outcome.warnings,
            vec![
                "Skipped skills and MCP configuration because no coding agents were selected or detected."
