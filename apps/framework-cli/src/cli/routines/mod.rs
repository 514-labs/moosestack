...
/// Prompts user to set up remote ClickHouse connection interactively
///
/// Returns:
/// - Ok(Some(config)) if user provided credentials and they were saved
/// - Ok(None) if user declined or suppressed prompts
/// - Err if operation failed
async fn prompt_remote_setup(
    project: &Project,
) -> Result<Option<crate::infrastructure::olap::clickhouse::config::ClickHouseConfig>, RoutineFailure>
{
    ...
    let setup_choice = prompt_user("Do you want to set this up now (Y/n)?", Some("Y"), None)?;

    if !matches!(
        setup_choice.trim().to_lowercase().as_str(),
        "" | "y" | "yes"
    ) {
        // User declined, ask about suppression
    ...
    // User wants to set up, prompt for URL
    let url = code_generation::prompt_user_for_remote_ch_http()?;

    // Parse URL first - don't store invalid URLs to keychain
    let config =
        match crate::infrastructure::olap::clickhouse::config::parse_clickhouse_connection_string(
            &url,
        ) {
            Ok(config) => config,
            Err(e) => {
                display::show_message_wrapper(
                    MessageType::Error,
                    Message::new("URL".to_string(), format!("Invalid ClickHouse URL: {e}")),
                );
                warn!("Failed to parse ClickHouse URL: {e:?}");
                return Ok(None);
            }
        };

    // URL is valid, now store to keychain
    let repo = KeyringSecretRepository;
    let project_name = project.name();

    if let Err(e) = repo.store(&project_name, KEY_REMOTE_CLICKHOUSE_URL, &url) {
        display::show_message_wrapper(
            MessageType::Error,
            Message::new(
                "Keychain".to_string(),
        ...
        warn!("Failed to store connection string to keychain: {e:?}");
        // Still return the config since parsing succeeded - user can use it this session
    } else {
        display::show_message_wrapper(
            MessageType::Success,
            Message::new(
...
pub async fn start_development_mode(
    ...
    if !externally_managed.is_empty() {
                ...
                Message {
                    ...
                    details: format!(
                        "Detected {} EXTERNALLY_MANAGED table(s) in your code.\n\
                         \n\
                         To enable Materialized Views and local development, add to moose.config.toml:\n\
                         \n\
        ...
        match resolve_remote_clickhouse_config(&project, None) {
            Ok(Some(remote_config)) => {
                ...
                match client.list_tables(&remote_config.db_name, &project).await {
                    ...
                    Err(e) => {
                        ...
                        // Fall back to creating mirrors from local schema (no remote data)
                        maybe_create_external_mirrors(
                            &project,
                            &plan.target_infra_map,
                            None, // Pass None to trigger local schema fallback
                        )
                        .await;
                    }
                }
            ...
            Ok(None) => {
                // No remote config, offer to set up interactively
                let setup_config = prompt_remote_setup(&project).await?;
                maybe_create_external_mirrors(
                    &project,
                    &plan.target_infra_map,
                    setup_config.as_ref(),
