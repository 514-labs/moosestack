...
async fn prompt_remote_setup(
    project: &Project,
    _settings: &Settings,
) -> Result<Option<crate::infrastructure::olap::clickhouse::config::ClickHouseConfig>, anyhow::Error>
{
    display::show_message_wrapper(
        MessageType::Info,
    ...
    match repo.store(&project_name, KEY_REMOTE_CLICKHOUSE_URL, &url) {
        Ok(()) => {
            ...
            match crate::infrastructure::olap::clickhouse::config::parse_clickhouse_connection_string(
                ...
                Err(e) => {
                    warn!("Failed to parse URL after storing: {e:?}");
                    Ok(None)
                }
            }
        }
    ...
    if !externally_managed.is_empty() {
        ...
        match code_generation::resolve_remote_clickhouse_config(&project, None) {
            ...
            Ok(None) => {
                ...
                let setup_config = prompt_remote_setup(&project, settings).await?;
                maybe_create_external_mirrors(
                    &project,
                    &plan.target_infra_map,
                    setup_config.as_ref(),
