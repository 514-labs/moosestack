...
pub async fn remote_plan(
    ...
    let remote_infra_map = if let Some(clickhouse_url) = clickhouse_url {
        ...
        let table_ids: std::collections::HashSet<String> = local_infra_map
            .tables
            .values()
            .map(|t| t.id(&local_infra_map.default_database))
            .collect();
        get_remote_inframap_serverless(project, clickhouse_url, None, table_ids).await?
    } else {
        // Moose server flow
        display::show_message_wrapper(
            MessageType::Info,
...
async fn get_remote_inframap_serverless(
    ...
    target_table_ids: std::collections::HashSet<String>,
