...
//! - **Format Layer**: Uses tracing-subscriber's native formatting (text or JSON)
...
pub struct LoggerSettings {
    ...
    /// When set, exports spans/logs to OTLP collector via gRPC with local file logging
    #[serde(default)]
...
pub fn setup_logging(settings: &LoggerSettings) {
    ...
    // When OTLP is enabled, set up both OTLP export AND local file logging
    if let Some(endpoint) = &settings.otlp_endpoint {
        setup_otlp_with_file_logging(settings, endpoint, env_filter);
        return;
    }
...
/// Sets up OTLP export with local file logging.
///
/// Creates both an OTLP bridge layer for remote export and a fmt layer for local file output.
fn setup_otlp_with_file_logging(settings: &LoggerSettings, endpoint: &str, env_filter: EnvFilter) {
    ...
    // Create local file layer (always without ANSI for file output)
    ...
    // Compose both OTLP and file layers
