...
export async function generateStaticParams() {
  const slugs = await getAllDocSlugs("typescript");
...
export async function generateMetadata({
  ...
  const doc = await getDocBySlug("typescript", slug);
...
export default async function TypeScriptDocPage({
  params,
}: {
  params: Promise<{ slug?: string[] }>;
}) {
  ...
  const breadcrumbs = await getBreadcrumbs("typescript", slug);
        ...
        <nav className="flex items-center space-x-2 text-sm text-muted-foreground mb-6">
          {breadcrumbs.map((crumb, index) => (
            <div key={crumb.slug.join("/")} className="flex items-center">
              ...
              <Link
                href={`/typescript/${crumb.slug.join("/")}`}
