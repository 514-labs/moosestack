 ...
 */
export function extractCodeSnippets(content: string, filePath: string): CodeSnippet[] {
  ...
  while ((match = codeBlockRegex.exec(content)) !== null) {
    ...
    if (directive?.includes("@test") || ["typescript", "javascript", "python"].includes(language)) {
      snippets.push({
        code,
        language,
        file: filePath,
  ...
  function processDirectory(dir: string) {
    ...
    for (const entry of entries) {
      ...
      if (entry.isDirectory()) {
        ...
        const snippets = extractCodeSnippets(body, path.relative(contentDir, fullPath));
 ...
 */
export async function validateTypeScriptSnippet(snippet: CodeSnippet): Promise<TestResult> {
  try {
 ...
 */
export async function validatePythonSnippet(snippet: CodeSnippet): Promise<TestResult> {
  try {
    const code = snippet.code.trim();
 ...
 * Test all snippets
 */
export async function testSnippets(snippets: CodeSnippet[]): Promise<TestResult[]> {
