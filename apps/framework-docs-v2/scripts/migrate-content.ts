 ...
 */
function extractLanguageContent(content: string, language: "typescript" | "python"): string {
  ...
  const otherTagRegex = new RegExp(`<${otherTag}[^>]*>([\\s\\S]*?)</${otherTag}>`, "gi");
  let processed = content.replace(otherTagRegex, "");

  // Extract this language's content (remove wrapper tags)
  const thisTagRegex = new RegExp(`<${languageTag}[^>]*>([\\s\\S]*?)</${languageTag}>`, "gi");
...
function main() {
  ...
  console.log("\n✓ Migration complete!");
  console.log("\nNext steps:");
  console.log("1. Review migrated content in content/typescript and content/python");
