...
|-------------|----------|
| **Add workflow** | Registered with Temporal (scheduled) or available for manual trigger |
| **Remove workflow** | Terminated in Temporal |
| **Rename workflow** | Old terminated, new registered |
| **Workflow config** | Old terminated, new registered |
| **Task code or config** | Picked up automatically — the worker restarts and uses the new code |
| **No changes** | Nothing happens — workers reconnect, existing workflows continue |
