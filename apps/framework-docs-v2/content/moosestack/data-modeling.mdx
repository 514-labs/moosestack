...
import { Callout, LanguageTabs, LanguageTabContent, CTACards, CTACard, BulletPointsCard, FileTree } from "@/components/mdx";
...
- [OLAP Tables and Materialized Views](/moosestack/olap) (automatically generated DDL)
- [Redpanda/Kafka Streams](/moosestack/streaming) (schema registry and topic validation)
- [API Contracts](/moosestack/apis) (request/response validation and OpenAPI specs)
- [Workflow Task Input and Output Types](/moosestack/workflows) (typed function inputs/outputs)
...
You can attach **ClickHouse table-level constraints** to an `OlapTable`
or to the **`table`** portion of an `IngestPipeline` config (when `table` is an object).

#### The `TableConstraint` type

Constraints are defined using the `TableConstraint` type:

| Field | Type | Purpose |
| ----- | ---- | ------- |
| `name` | `string` | Unique constraint name in ClickHouse. |
| `expression` | `string` | SQL predicate (what appears inside the constraint body in DDL). |
| `type` | `"CHECK"` \| `"ASSUME"` | Constraint kind (see below). |

In TypeScript this is the `TableConstraint` interface from `@514labs/moose-lib`. In Python it is `OlapConfig.TableConstraint` (nested class on `OlapConfig`).

##### `type`: `CHECK` vs `ASSUME`

- **`CHECK`** — ClickHouse **validates** this predicate on **INSERT** (and similar write paths) **when the engine applies constraint checking** for that table. If validation does not apply (for example, read-only or external engines where you never insert into that table), the constraint may still appear in DDL but has nothing to enforce at insert time.
- **`ASSUME`** — **Optimizer-only**: ClickHouse **does not** enforce this on insert. It tells the planner that the predicate always holds so it may rewrite queries (e.g. replace expressions with materialized columns). If the data violates the assumption, **query results can be wrong**; use only when you are sure the property holds.

#### `OlapTable` example

<LanguageTabs>
  <LanguageTabContent value="typescript">
```ts filename="app/datamodels/user.ts"
import { Key, OlapTable, TableConstraint } from "@514labs/moose-lib";
...
const ageConstraint: TableConstraint = {
  name: "valid_age",
  expression: "age >= 0",
  type: "CHECK",
};

const nameLenConstraint: TableConstraint = {
  name: "assume_name_len",
  expression: "length(name) = name_len",
  type: "ASSUME",
};

export const UserTable = new OlapTable<User>("User", {
  constraints: [ageConstraint, nameLenConstraint],
});
```
  </LanguageTabContent>
  <LanguageTabContent value="python">
```python
from moose_lib import OlapTable, OlapConfig, Key, MergeTreeEngine, MooseModel
...
age_constraint = OlapConfig.TableConstraint(
    name="valid_age",
    expression="age >= 0",
    type="CHECK",
)

name_len_constraint = OlapConfig.TableConstraint(
    name="assume_name_len",
    expression="length(name) = name_len",
    type="ASSUME",
)
...
UserTable = OlapTable(
    ...
    OlapConfig(
        ...
        constraints=[age_constraint, name_len_constraint],
    ),
)
```
  </LanguageTabContent>
...
#### `IngestPipeline` example

Pass constraints on the **`table`** key alongside engine and `orderByFields` / `orderByExpression` as usual:

<LanguageTabs>
  <LanguageTabContent value="typescript">
```ts filename="app/datamodels/events_pipeline.ts"
import {
  Key,
  IngestPipeline,
  ClickHouseEngines,
...
const nonNegative: TableConstraint = {
  name: "value_non_negative",
  expression: "value >= 0",
  type: "CHECK",
};

export const eventsPipeline = new IngestPipeline<Event>("events", {
  ingestApi: true,
  stream: true,
  table: {
...
```
  ...
  <LanguageTabContent value="python">
```python filename="app/datamodels/events_pipeline.py"
...
class Event(MooseModel):
    id: Key[str]
    value: float

value_non_negative = OlapConfig.TableConstraint(
    name="value_non_negative",
    expression="value >= 0",
    type="CHECK",
)

events_pipeline = IngestPipeline[Event](
    "events",
    IngestPipelineConfig(
        ingest_api=True,
        stream=True,
        table=OlapConfig(
            engine=MergeTreeEngine(),
            order_by_fields=["id"],
            constraints=[value_non_negative],
        ),
    ),
)
```
...
#### ASSUME and the query optimizer

For `ASSUME`, ClickHouse may rewrite queries using the declared predicate—for example, if `ASSUME` ties a function to a stored column, filters using the function might be turned into filters on that column. **`ASSUME` does not validate rows on insert**; only **`CHECK`** does, on engines/paths where insert-time checking applies.
...
import { Key, Decimal, ClickHouseDecimal, LowCardinality, ClickHouseNamedTuple } from "@514labs/moose-lib";

export interface ClickHouseOptimized {
  id: Key<string>;

  // ClickHouse-specific type annotations
  amount: Decimal<10, 2>;                       // Decimal(10,2) in ClickHouse
  // Alternative: amount: string & ClickHouseDecimal<10, 2>; // Verbose syntax still works
...
import { tags, Api } from "@514labs/moose-lib";
...
const searchAPI = new Api<SearchParams, SearchResult[]>(
  "search",
  async (params, { client }) => {
...
// API Usage Examples:
// ✅ Valid: GET /api/search?startDate=2024-01-01&endDate=2024-01-31
// ✅ Valid: GET /api/search?startDate=2024-01-01&endDate=2024-01-31&minValue=100&limit=50
// ❌ Invalid: GET /api/search?startDate=Jan-1-2024 (wrong date format)
// ❌ Invalid: GET /api/search?startDate=2024-01-01&endDate=2024-01-31&limit=200 (exceeds max)
```
  </LanguageTabContent>
  <LanguageTabContent value="python">
...
const exampleTask = new Task<TaskInput, TaskOutput>(
  "example-task",
  {
    run: async (ctx) => {
      ...
      if (ctx.input.taskType === "typeA") {
        output.resultA = await processTypeA(ctx.input);
      }
