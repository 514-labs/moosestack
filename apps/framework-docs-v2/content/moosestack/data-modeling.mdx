...
You can configure constraints on MergeTree-family ClickHouse tables to enforce data integrity. 

#### The `TableConstraint` Interface

Constraints are defined using the `TableConstraint` interface:

- `name` (string): The identifier for the constraint.
- `expression` (string): The ClickHouse SQL expression to evaluate.
- `type` (string): The type of constraint. Allowed values are `"CHECK"` or `"ASSUME"`.

*Compatibility Note: Table constraints are fully supported in both standalone `OlapTable` definitions and when used inside `IngestPipeline` configurations.*
...
# Defining constraints via OlapConfig.TableConstraint
...
In the example above, `ASSUME CONSTRAINT` is used to assert that the `length(name)` function always equals the value of the `name_len` column. This means that whenever `length(name)` is called in a query, ClickHouse can replace it with `name_len`, which should be faster because it avoids calling the `length()` function.

Then, when executing a query like `SELECT name FROM User WHERE length(name) < 5;`, ClickHouse can optimize it to `SELECT name FROM User WHERE name_len < 5;` because of the `ASSUME CONSTRAINT`. This can make the query run faster because it avoids calculating the length of name for each row.

**Note**: `ASSUME CONSTRAINT` does not enforce the constraint, it merely informs the optimizer that the constraint holds true. If the constraint is not actually true, the results of the queries may be incorrect. Therefore, you should only use `ASSUME CONSTRAINT` if you are sure that the constraint is true.
