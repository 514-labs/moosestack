...
- **Enhanced SQL template tag with join, raw, and append helpers** ([@okane16](https://github.com/okane16))
  ...
  **Dynamic query building with SQL template helpers**
  ...
  ```typescript
  import { sql, type Sql } from "@514labs/moose-lib";
  ...
  export function buildUserQuery(filters: {
    ...
    const columns = [
      ...
      sql`${sql.raw("NOW()")} AS query_time`,
    ];

    const where: Sql[] = [];
    if (filters.minAge !== undefined)
      where.push(sql`${UserTable.columns.age} >= ${filters.minAge}`);
    if (filters.status)
      where.push(sql`${UserTable.columns.status} = ${filters.status}`);

    let query = sql`SELECT ${sql.join(columns, ", ")} FROM ${UserTable}`;
    if (where.length) query = query.append(sql` WHERE ${sql.join(where, " AND ")}`);

    return query.append(
      sql` ORDER BY ${UserTable.columns.created_at} ${sql.raw("DESC")} LIMIT ${100}`,
    );
  }
  ```

  This example demonstrates building dynamic SQL queries using the new sql template helpers: `sql.join()` to combine column lists and WHERE conditions, `sql.raw()` to insert SQL like `NOW()`, and `Sql.append()` to chain query parts together incrementally.
