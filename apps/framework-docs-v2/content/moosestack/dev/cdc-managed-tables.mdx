...
- Your ClickHouse connection URL (e.g., `clickhouse://user:pass@host:8443/database?secure=true`)
...
- Automatically detects CDC-managed tables and marks them appropriately
- Saves your connection config to `moose.config.toml`
- Stores credentials securely in your OS keychain

```bash
moose init my-analytics-app --from-remote "clickhouse://user:pass@host:8443/database?secure=true" --language typescript
```

You'll see output like:

```
   Creating my-analytics-app from typescript-empty template
    Success Project created at my-analytics-app
   Protocol native protocol detected. Converting to HTTP(s): https://host:8443
  Highlight Table(s) with types unsupported: _system_tables
...
After initialization, navigate into your project:
...
By default, when you run `moose dev`, externally managed tables don't exist locally—they only exist in your remote ClickHouse. This means any views or queries that reference them will fail.

To develop locally, enable local mirrors:
...
3. **Manually trigger with `moose seed`** - Insert test data after startup:
   ```bash
...
Credentials are stored in your OS keychain and reused automatically. To skip prompts, set environment variables:
...
1. **Credentials resolved** from keychain or environment
2. **Remote connected** to fetch schemas and sample data
3. **Local mirrors created** matching remote table structures
4. **Data seeded** (if `sample_size > 0`)
...
- Creates a git commit if anything changed
...
$ moose db pull
   ...
   Introspecting tables...
        Info External models refreshed (1 table updated)
         Git created commit a1b2c3d
```

Your `externalModels.ts` now includes:
...
| Task | Command |
|:-----|:--------|
| Start new project from existing ClickHouse | `moose init my-app --from-remote <URL> --language typescript` |
...
- [Materialized Views](/moosestack/olap/materialized-views) - Build real-time aggregations on CDC data
- [Consumption APIs](/moosestack/consumption-apis) - Expose your data through type-safe endpoints
