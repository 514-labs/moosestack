...
1. **Remote connected** - Moose connects to your remote ClickHouse
2. **Local tables created** - Mirror tables are created in local ClickHouse matching remote schemas
3. **Data seeded** - If `sample_size > 0`, rows are copied from remote for local testing
4. **Server starts** - Your APIs and views work against local data
