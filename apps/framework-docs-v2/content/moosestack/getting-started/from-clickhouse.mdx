...
When `moose init --from-remote` imports replicated ClickHouse tables, Moose strips the remote keeper path and replica name from the generated model definitions. Those values are specific to the source cluster, so local development regenerates the replication parameters at table creation time instead of reusing remote UUID-based paths.
</Callout>
