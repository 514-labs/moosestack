...
import { LanguageTabs, LanguageTabContent, FeatureCards, FeatureCard } from "@/components/mdx";

# Add Moose to an Existing Application

Bring type-safe ClickHouse migrations and/or queries into an app you already run (Next.js, Express, FastAPI, or any other server) without adopting the full runtime. You'll install the lightweight libraries, point Moose OLAP at your managed ClickHouse cluster, define tables as code, run migrations directly against production, and query the data from your existing API layer.
