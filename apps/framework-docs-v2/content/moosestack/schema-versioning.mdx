...
description: Use the `version` field on Moose primitives to model breaking schema changes and coordinate cutovers
order: 1
category: reference
---
...
For command options and connection modes, see [Generate migrations](/moosestack/migrate/generate) and the [`moose generate migration` CLI reference](/moosestack/moose-cli).
...
- [OlapTable](/moosestack/olap/model-table)
