...
This integrated approach provides the best experience for most applications. However, if you only need ClickHouse schema management and type-safe queries inside an existing app, start with the guides for [Embed ClickHouse analytics in Next.js](/moosestack/getting-started/existing-app/next-js) or [Embed ClickHouse analytics in Fastify](/moosestack/getting-started/existing-app/fastify).
...
*   **[Networking Reference](/moosestack/configuration/dev-environment#networking-reference)**: See the default ports and URLs Moose uses.
