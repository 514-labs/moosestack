...
**Output options:**
- `--save`: Write `plan.yaml`, `remote_state.json`, and `local_infra_map.json` to the migrations directory. Omit to output plan YAML to stdout.
- `--output-dir <DIR>`: Write migration artifacts to a custom directory instead of the default. Overrides `migration_config.dir`.
