...
```bash
moose build [--docker] [--dockerfile] [--amd64] [--arm64]
```
- `--docker`: Generate a Dockerfile and build Docker image(s)
- `--dockerfile`: Generate the Dockerfile only, without building the Docker image (requires `custom_dockerfile = true` in `moose.config.toml`)
