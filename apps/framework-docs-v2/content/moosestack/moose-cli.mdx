...
- `--yes-destructive`: Skip the interactive confirmation prompt for destructive operations (column drops, table drops/recreates). Useful for CI/CD or when you intentionally want to accept all destructive schema changes. Can also be set via the `MOOSE_ACCEPT_DESTRUCTIVE=1` environment variable.
