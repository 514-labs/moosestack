...
# Add a standalone benchmark package to a TypeScript Moose project
moose add benchmark [--dir <moose-project-dir>]
```
...
The `benchmark` component copies a standalone `query-benchmarks/` package into your Moose project. After `moose add benchmark`, install its dependencies inside that directory, edit `query-benchmarks/benchmark.config.ts` to import your compiled query model, define the base query and scenarios, run `moose build` in the parent project, then run the benchmark package scripts from `query-benchmarks/`.
