...
```bash
moose harness init
moose harness init <name> <template> [--location <location>] [--agent <agent>] [--from-remote <connection-string>] [--lsp]
```

- Zero-arg `moose harness init` starts the interactive wizard.
- Arg-driven mode is non-interactive: if you pass flags or positionals, omitted optional values fall back to defaults instead of prompting.
- `--agent`: Repeat to target specific coding agents. Omit to auto-detect supported agents.
- `--from-remote`: Bootstrap the project from a remote ClickHouse cluster.
- `--lsp`: Install and configure the MooseStack LSP where supported.
...
```bash
moose dev [--mcp] [--no-infra] [--timestamps] [--timing] [--log-payloads] [--yes-destructive]
```
...
- `--yes-destructive`: Skip the interactive confirmation prompt for destructive operations (table, column, view, and materialized-view removals). Useful for CI/CD or when you intentionally want to accept all destructive schema changes. Can also be set via the `MOOSE_ACCEPT_DESTRUCTIVE=1` environment variable.
