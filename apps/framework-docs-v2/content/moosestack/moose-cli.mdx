...
### Add
Adds a pre-built component to an existing project. Downloads the component files from the template registry and installs dependencies.
...
- Must be run from (or point `--dir` at) a Moose project directory
- TypeScript projects only

**`chat` prerequisites:**
- Must be run from (or point `--dir` at) a Next.js app directory
- Run `pnpm dlx shadcn@latest init` (or `npx shadcn@latest init`) first if you haven't initialized shadcn/ui
