...
### Add
Adds a pre-built component to an existing project. Downloads the component files from a registry and installs dependencies.
...
Runs a lightweight stdio-based MCP proxy server for always-on AI agent integration. Unlike the HTTP MCP server bundled in `moose dev`, this process starts immediately without requiring the dev server to be running. When AI agents send tool calls, they are proxied to the dev server if it is available; if not, the proxy returns a clear message instructing the agent to start `moose dev`.

```bash
moose mcp [--host <HOST>] [--port <PORT>]
```

- `--host`: Host of the dev server to proxy to (default: `localhost`, auto-detected from project config if omitted)
- `--port`: Port of the dev server to proxy to (default: `4000`, auto-detected from project config if omitted)

The proxy advertises the same five tools as the dev MCP server (`get_infra_map`, `get_logs`, `query_olap`, `get_stream_sample`, `get_issues`) and communicates over stdio, which is natively supported by most AI clients.
...
**Manual configuration (stdio transport):**
...
See [MCP Server documentation](/moosestack/moosedev-mcp) for full setup instructions across AI clients.
...
- `--limit`: Copy up to N rows (mutually exclusive with `--all`). Overrides per-table `seedFilter.limit`; when omitted falls back to `seedFilter.limit`, then 1000. Large limits are automatically batched.
...
**Per-table seed filters:** Tables can declare a `seedFilter` in their `OlapTable` config to set a default limit and/or WHERE clause when seeding. See [Seed Filter](/moosestack/olap/model-table#seed-filter) for details.
