...
```toml filename="moose.config.toml"
[migration_config]
# Default: true — require a reviewed plan.yaml for destructive changes
...
2. Set `require_plan_for_destructive = false` in `moose.config.toml` (or `MOOSE_MIGRATION_CONFIG__REQUIRE_PLAN_FOR_DESTRUCTIVE=false` as an environment variable) to opt out of the safety gate.

<Callout type="warning" title="Opting out">
  Setting `require_plan_for_destructive = false` restores the pre-safety-gate behavior where `moose prod` applies destructive changes without a plan file. Only do this if you accept the risk of unreviewed destructive schema changes in production.
</Callout>
