...
| `--save` | Writes the generated plan to the `migrations/` directory (or the directory configured in `migration_config.dir`). Without this, it performs a dry run. |
| `--output-dir` | Custom output directory for migration artifacts. Overrides `migration_config.dir`. |
