...
Migration operations apply to ClickHouse OLAP tables (including materialized views) defined in your Moose project. Streams, APIs, and workflows currently bypass `plan.yaml` and are provisioned directly by Moose tooling.

Ensure the OLAP feature flag is enabled in `moose.config.toml`:
