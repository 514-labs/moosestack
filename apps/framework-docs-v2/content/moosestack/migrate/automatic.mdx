...
By default, `moose prod` blocks startup when destructive changes are detected without a reviewed `plan.yaml`. See [Production Safety Gate](/moosestack/migrate/planned-migrations#production-safety-gate) for details. To override at runtime without changing config, set `MOOSE_MIGRATION_CONFIG__REQUIRE_PLAN_FOR_DESTRUCTIVE=false`.
