...
This guide provides detailed examples of different migration types. For the complete workflow overview, see [Migrations & Planning](/moosestack/migrate).
...
<Callout type="info" title="Understanding OLAP Migrations" href="/moosestack/olap/apply-migrations" ctaLabel="Learn More">
...
Normally, Moose validates cluster identifiers strictly. However, if an identifier is wrapped in braces (e.g. `{cluster}`), Moose treats it as a ClickHouse macro. It bypasses local static validation and defers evaluation to the ClickHouse server at execution time. 

<Callout type="info" title="Macro Syntax Verification">
While validation is bypassed for the full macro name, Moose will still strictly verify that the braces are well-formed (no mismatched or unbalanced brackets like `}{`) to prevent malformed queries from reaching ClickHouse.
</Callout>
...
When this migration is applied, the CLI dynamically generates DDL using ClickHouse's single-quote interpolation for macros:

```sql
CREATE TABLE `user_events` ON CLUSTER '{cluster}' (id String) ENGINE = ReplicatedMergeTree;
```

ClickHouse reads the single-quoted string `'{cluster}'` during execution, detects the `{...}` syntax, and substitutes it with the `cluster` macro value defined in its local `macros.xml` configuration file.
