...
```typescript filename="FlexiblePayload.ts" copy
...
- Known fields are passed through to streaming functions for processing

<Callout type="info" title="Current Behavior">
Index signatures allow the API to accept extra fields gracefully. However, only the known (declared) fields are currently passed through to streaming functions. If you need to capture and process arbitrary fields, consider using a `ClickHouseJson` field type which stores the entire payload as JSON.
</Callout>
  </LanguageTabContent>
  <LanguageTabContent value="python">
...
```python filename="FlexiblePayload.py" copy
from pydantic import BaseModel, ConfigDict
from moose_lib import IngestApi, Stream, Key
