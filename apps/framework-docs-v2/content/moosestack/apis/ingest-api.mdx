...
import { Callout, LanguageTabs, LanguageTabContent, BulletPointsCard, ZoomImg, CompareBulletPointsCard, CTACard, CTACards, Tabs, TabsList, TabsTrigger, TabsContent } from "@/components/mdx";
...
<Callout type="info" title="Optional fields with ClickHouse defaults in IngestPipeline">
...
In some scenarios, you may need to accept payloads with arbitrary additional fields beyond your defined schema. This is common when:
- You don't control the payload structure from an upstream service
- You want to capture dynamic event properties that vary by event type
- You need to store extra fields in a JSON column for later processing

<LanguageTabs>
  <LanguageTabContent value="typescript">
### Using Index Signatures (TypeScript)

TypeScript's index signatures allow you to accept any additional properties while still defining known fields:

```typescript filename="DynamicPayload.ts" copy
import { IngestApi, Stream, OlapTable, Key, DateTime, ClickHouseJson } from "@514labs/moose-lib";

// Input type with known fields + index signature for arbitrary fields
type UserEventInput = {
  timestamp: DateTime;
  eventName: string;
  userId: Key<string>;
...
// Output type with arbitrary fields collected into a JSON column
interface UserEventOutput {
  timestamp: DateTime;
  eventName: string;
  userId: Key<string>;
  orgId?: string;
  // All extra fields stored in a JSON column
  properties: Record<string, any> & ClickHouseJson;
}

// Set up the pipeline
const outputTable = new OlapTable<UserEventOutput>("UserEventOutput", {
  orderByFields: ["userId", "timestamp"],
});

const outputStream = new Stream<UserEventOutput>("UserEventOutput", {
  destination: outputTable,
});

const inputStream = new Stream<UserEventInput>("UserEventInput");

// IngestApi accepts payloads with extra fields
const ingestApi = new IngestApi<UserEventInput>("user-events", {
  destination: inputStream,
});

// In your streaming function, extract known fields and collect the rest:
inputStream.addTransform(outputStream, (input) => {
  ...
  return {
    ...
    userId,
    orgId,
    properties: extraFields, // Arbitrary fields go to JSON column
  };
});
```
...
- Additional fields matching the index signature are passed through without validation
- Use a streaming function to transform the input and collect extra fields into a JSON column
  </LanguageTabContent>
  <LanguageTabContent value="python">
...
```python filename="DynamicPayload.py" copy
from pydantic import BaseModel, ConfigDict
from typing import Any
from moose_lib import IngestApi, Stream, OlapTable, Key, ClickHouseJson
...
# Output model with JSON column for extra fields
class UserEventOutput(BaseModel):
    timestamp: datetime
    event_name: str
    user_id: Key[str]
    org_id: str | None = None
    properties: dict[str, Any]  # Annotate with ClickHouseJson for typed paths

# Set up the pipeline
output_table = OlapTable[UserEventOutput]("UserEventOutput", OlapConfig(
    order_by_fields=["user_id", "timestamp"]
))

output_stream = Stream[UserEventOutput]("UserEventOutput", StreamConfig(
...
# In your streaming function, access extra fields via model_extra:
@input_stream.add_transform(output_stream)
def transform(input: UserEventInput) -> UserEventOutput:
    return UserEventOutput(
        timestamp=input.timestamp,
        event_name=input.event_name,
        user_id=input.user_id,
        org_id=input.org_id,
        properties=input.model_extra or {},  # Pydantic stores extra fields here
    )
```

**How it works:**
- `model_config = ConfigDict(extra='allow')` tells Pydantic to accept undeclared fields
- Extra fields are accessible via `input.model_extra` dictionary
- Use a streaming function to collect extra fields into a JSON column
  </LanguageTabContent>
</LanguageTabs>

<Tabs>
  <TabsList>
    <TabsTrigger value="valid">Valid Event</TabsTrigger>
    <TabsTrigger value="missing">Invalid: Missing Field</TabsTrigger>
    <TabsTrigger value="date">Invalid: Bad Date</TabsTrigger>
  </TabsList>
...
fetch("http://localhost:4000/ingest/your-api-route", {
  method: "POST",
  body: JSON.stringify({
    id: "event1",
    userId: "user1",
...
const exampleTable = new OlapTable<ExampleRecord>("example-table-name");

// Create the stream with specific settings
const exampleStream = new Stream<ExampleRecord>("example-stream-name", {
  destination: exampleTable    // Connect stream to table
});

// Create the ingestion API
const exampleApi = new IngestApi<ExampleRecord>("example-api-route", {
  destination: exampleStream,  // Connect API to stream
});
```
  </LanguageTabContent>
  <LanguageTabContent value="python">
```python filename="AnalyticsPipelineManual.py" copy
...
<Tabs>
  <TabsList>
    <TabsTrigger value="pipeline">IngestPipeline</TabsTrigger>
    <TabsTrigger value="api">IngestApi</TabsTrigger>
  </TabsList>
