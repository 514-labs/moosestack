...
const query = sql`SELECT *, ${timestamp} as query_time FROM ${UserTable}`;
// Result: SELECT *, NOW() as query_time FROM `users`

// Sort direction
const direction = sql.raw("DESC");
const sortedQuery = sql`SELECT * FROM ${UserTable} ORDER BY created_at ${direction}`;
...
const buildQuery = ({ minAge, status, limit = 100 }: QueryParams, { sql }) => {
  ...
  query = query
    .append(sql` ORDER BY created_at ${sql.raw("DESC")}`)
...
const buildQuery = ({ minAge, status, searchText }: FilterParams, { sql }) => {
  ...
  query = query.append(sql` ORDER BY created_at ${sql.raw("DESC")}`);
  return query;
};
```
  </LanguageTabContent>
