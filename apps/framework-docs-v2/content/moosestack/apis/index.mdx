...
If you want to **embed Moose as a library inside an existing web app** (no separate Moose Runtime service), follow the “Add to Existing App” guides like [Next.js](/moosestack/getting-started/existing-app/next-js).
...
APIs are where external systems touch your data platform. Moose gives you a path that is:
...
| Add ClickHouse modeling + queries inside an existing app | **Client-only embedding** (e.g. [Next.js](/moosestack/getting-started/existing-app/next-js)) |

### 1) Native Moose APIs

Native APIs are best for straightforward endpoints with minimal routing/middleware needs.

- **Ingest**: create POST endpoints that validate payloads and route them into streams/tables.
- **Analytics**: create GET endpoints that run queries (OLAP or custom logic).
- **Trigger**: create POST endpoints that kick off workflows.
...
### 3) Embed Moose in an existing app (client-only)

If you already have an app server (like Next.js), you can keep it where it is and use Moose libraries to model, migrate, and query ClickHouse **without deploying a separate Moose Runtime web service**.

See: [Embed ClickHouse analytics in Next.js](/moosestack/getting-started/existing-app/next-js).

## Key concepts

### WebApp utilities (Moose Runtime hosting)

When MooseStack hosts your framework, your handlers can access Moose utilities from the current request.
...
### Mount paths (for embedded frameworks)
