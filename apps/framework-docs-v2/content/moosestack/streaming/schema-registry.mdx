...
description: Publish a `Stream` through Confluent Schema Registry with `schemaConfig` or `schema_config`
order: 7
category: streaming
---
...
Use `schemaConfig` (TypeScript) or `schema_config` (Python) to publish a [Stream](/moosestack/streaming/create-stream) through Confluent Schema Registry.
