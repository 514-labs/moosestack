...
# When true (default), `moose prod` refuses to start if the computed diff
# contains destructive operations and no plan.yaml is present.
# Set to false to allow unplanned destructive changes in production.
...
### Migrations
```bash
MOOSE_MIGRATION_CONFIG__REQUIRE_PLAN_FOR_DESTRUCTIVE=true
```
