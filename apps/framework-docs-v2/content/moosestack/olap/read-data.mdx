...
When you query through `MooseClient.query.execute`, [ALIAS](/moosestack/olap/alias-columns) and [MATERIALIZED](/moosestack/olap/materialized-columns) columns are automatically included in `SELECT *` results. This means your typed results will contain all computed column values without needing to name them explicitly.

<Callout type="info" title="Custom SQL outside QueryClient">
If you write custom SQL using the ClickHouse client directly (outside of `MooseClient.query`), ALIAS and MATERIALIZED columns are excluded from `SELECT *` by default (standard ClickHouse behavior). You can either name them explicitly in your SELECT clause or set `asterisk_include_alias_columns=1` and `asterisk_include_materialized_columns=1` in your query settings.
</Callout>
