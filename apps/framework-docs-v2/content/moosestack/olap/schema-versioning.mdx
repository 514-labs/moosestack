...
| Flag | Effect |
| :--- | :--- |
| `--yes-all` | Auto-accept backfill (if eligible) and drop old tables. Equivalent to `MOOSE_ACCEPT_ALL=1`. |
| `--yes-destructive` | Auto-accept destructive changes including old table drops. Equivalent to `MOOSE_ACCEPT_DESTRUCTIVE=1`. |
