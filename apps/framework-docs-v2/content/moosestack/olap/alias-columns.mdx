...
ALIAS columns are computed by ClickHouse — you should **never** supply values for them when inserting data. Moose handles this automatically across all ingest paths (streams, HTTP ingest, and direct `OlapTable.insert()`).

<LanguageTabs>
  <LanguageTabContent value="typescript">
Moose exports an `Insertable<T>` utility type that derives the insert-safe shape of your model:
- **ALIAS columns are omitted** — they don't appear in the type at all
- **DEFAULT columns become optional** — ClickHouse fills them when absent

```ts filename="InsertableExample.ts" copy
import { OlapTable, Key, DateTime, ClickHouseAlias, ClickHouseDefault, Insertable } from "@514labs/moose-lib";
...
// Insertable<UserEvents> ≡ { id: string; timestamp: Date; userId: string; createdAt?: Date }
...
When you query data through Moose's `QueryClient` (via `MooseClient.query.execute`), ALIAS columns are automatically included in `SELECT *` results. This means your typed query results will contain the computed ALIAS values without needing to name them explicitly.

If you write custom SQL outside of `QueryClient`, ALIAS columns are excluded from `SELECT *` by default (standard ClickHouse behavior). Name them explicitly or set `asterisk_include_alias_columns=1` in your query settings.
