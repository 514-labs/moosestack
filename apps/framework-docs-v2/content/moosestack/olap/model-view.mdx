...
from moose_lib import View, ViewConfig
...
export const rawEvents = new View("raw_events", {
  selectStatement: sql.statement`SELECT * FROM ${events}`,
  baseTables: [events],
  database: "analytics",
});
```
...
// new View(name: string, config: ViewConfig)
new View("view_name", {
  selectStatement: sql.statement`SELECT ... FROM ${someTable}`,
  baseTables: [someTable /* , other tables or views */],
...
# View(name: str, config: ViewConfig)
View(
    "view_name",
    ViewConfig(
        select_statement="SELECT ... FROM {someTable}",
        base_tables=[someTable],
        database="optional_db",  # optional
    ),
)
```
