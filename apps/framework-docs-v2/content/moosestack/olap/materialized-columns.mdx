...
- **MATERIALIZED columns are omitted** — they don't appear in the type at all
- **DEFAULT columns become optional** — ClickHouse fills them when absent

```ts filename="InsertableExample.ts" copy
import { OlapTable, Key, DateTime, ClickHouseMaterialized, ClickHouseDefault, Insertable } from "@514labs/moose-lib";
...
When you query data through Moose's `QueryClient` (via `MooseClient.query.execute`), MATERIALIZED columns are automatically included in `SELECT *` results. This means your typed query results will contain the pre-computed MATERIALIZED values without needing to name them explicitly.

If you write custom SQL outside of `QueryClient`, MATERIALIZED columns are excluded from `SELECT *` by default (standard ClickHouse behavior). Name them explicitly or set `asterisk_include_materialized_columns=1` in your query settings.
