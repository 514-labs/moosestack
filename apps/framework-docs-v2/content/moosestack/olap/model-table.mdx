...
<Callout type="info" title="Development Mode Configuration for External Tables">
When working with EXTERNALLY_MANAGED tables (especially as sources for Materialized Views), configure automatic local mirroring in `moose.config.toml`:
...
This creates local copies with sample data, enabling Materialized Views to work in development. See [External Tables](/moosestack/olap/external-tables#development-mode-configuration) for complete details.
</Callout>

<Callout type="info" title="Learn More About LifeCycle Management">
Learn more about the different lifecycle options and how to use them in the [LifeCycle Management](/moosestack/migrate/lifecycle-externally-managed) documentation.
</Callout>
