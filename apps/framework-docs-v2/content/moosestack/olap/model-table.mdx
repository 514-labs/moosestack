...
When working with EXTERNALLY_MANAGED tables (especially as sources for MaterializedViews), configure automatic local mirroring in `moose.config.toml`:
...
This creates local copies with sample data, enabling MaterializedViews to work in development. See [External Tables](/moosestack/olap/external-tables#development-mode-configuration) for complete details.
</Callout>
