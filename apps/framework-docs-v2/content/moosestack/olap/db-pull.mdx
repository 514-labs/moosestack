...
- **--clickhouse-url**: Optional. ClickHouse URL (e.g., `clickhouse://user:pass@host:port/database` or `https://user:pass@host:port/database`). If not provided, the URL is resolved in this order:
  1. `CLICKHOUSE_URL` environment variable
...
### Automatically run on dev startup (keep local fresh)
  ...
  - Saved URL from `moose init --from-remote`

  To run after code reloads, use `on_reload_complete_script`.
