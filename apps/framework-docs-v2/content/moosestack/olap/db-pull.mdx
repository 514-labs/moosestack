...
<Callout type="info">This is a read-only sync for your code models. For concepts and modeling guidance, see [External Tables](/moosestack/olap/external-tables). To bootstrap a project from an existing DB, see [Initialize from ClickHouse](/moosestack/getting-started/from-clickhouse).</Callout>
...
moose db pull --clickhouse-url "clickhouse://explorer@play.clickhouse.com:9440?database=default"
...
- **--clickhouse-url**: ClickHouse URL (e.g., `clickhouse://user:pass@host:port?database=mydb` or `https://user:pass@host:port?database=mydb`). Can also be set via `CLICKHOUSE_URL` environment variable.
...
### Automatically run on dev startup (keep local fresh)
  ...
  ```bash filename="Terminal" copy
  export CLICKHOUSE_URL="clickhouse://<user>:<password>@<host>:<port>?database=<database>"
  ```
...
- **External Tables**: <a href="/moosestack/olap/external-tables">concepts and configuration</a>
- **Initialize from ClickHouse**: <a href="/moosestack/getting-started/from-clickhouse">bootstrap projects from an existing DB</a>
- **Supported Types**: <a href="/moosestack/olap/supported-types">mapping and constraints</a>
