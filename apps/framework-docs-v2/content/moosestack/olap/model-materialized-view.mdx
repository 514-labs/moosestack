...
Materialized views are write-time transformations in ClickHouse. A static `SELECT` populates a destination table from one or more sources. You query the destination like any other table. The `MaterializedView` class wraps [ClickHouse `MATERIALIZED VIEW`](https://clickhouse.com/docs/en/sql-reference/statements/create/view/#create-materialized-view) and keeps the `SELECT` explicit. When you edit the destination schema in code and update the `SELECT` accordingly, Moose applies the corresponding DDL, orders dependent updates, and backfills in dev mode, so the pipeline stays consistent as you iterate.
...
<Callout type="info" title="Dependency awareness" compact icon="Bulb">
...
<Callout type="info" title="Materialized View is like a trigger" compact icon="Bulb">
...
<LanguageTabs>
  <LanguageTabContent value="typescript">
```typescript filename="ViewOptions.ts" copy
import type { Sql, LifeCycle } from "@514labs/moose-lib";
...
interface MaterializedViewConfig<T> {
  ...
  // Controls how Moose manages the MV when code changes (default: FULLY_MANAGED)
  lifeCycle?: LifeCycle;
...
from moose_lib import MaterializedViewOptions, LifeCycle
from source_table import source_table
...
class MaterializedViewOptions(BaseModel):
    ...
    life_cycle: Optional[LifeCycle] = None  # Controls how Moose manages the MV (default: FULLY_MANAGED)
```
  </LanguageTabContent>
</LanguageTabs>
...
The `lifeCycle` option (TypeScript) / `life_cycle` option (Python) controls how Moose handles a materialized view when your code definitions change.

| Value | Create | Update (SELECT change) | Remove from code |
|---|---|---|---|
| `FULLY_MANAGED` (default) | ✅ Auto-created | ✅ Auto-updated (DROP + CREATE) | ✅ Auto-dropped |
| `DELETION_PROTECTED` | ✅ Auto-created | ❌ Blocked | ❌ Blocked |
| `EXTERNALLY_MANAGED` | ❌ Blocked | ❌ Blocked | ❌ Blocked |

<Callout type="info" title="MV updates require DROP + CREATE" compact>
ClickHouse cannot ALTER a materialized view's SELECT in place. When Moose detects a SELECT change, it drops and recreates both the MV and its target table. `DELETION_PROTECTED` and `EXTERNALLY_MANAGED` lifecycles block this operation to protect data.
</Callout>

<LanguageTabs>
  <LanguageTabContent value="typescript">
```ts filename="LifecycleMV.ts" copy
import { MaterializedView, LifeCycle, sql } from "@514labs/moose-lib";

// DELETION_PROTECTED: Moose will auto-create but never drop or update
export const protectedMV = new MaterializedView<TargetSchema>({
  selectStatement: sql.statement`SELECT id, value FROM ${sourceTable}`,
  selectTables: [sourceTable],
  targetTable: { name: "protected_target", orderByFields: ["id"] },
  materializedViewName: "protected_mv",
  lifeCycle: LifeCycle.DELETION_PROTECTED,
});

// EXTERNALLY_MANAGED: Moose will not touch this MV at all
export const externalMV = new MaterializedView<TargetSchema>({
  selectStatement: sql.statement`SELECT id, value FROM ${sourceTable}`,
  selectTables: [sourceTable],
  targetTable: { name: "external_target", orderByFields: ["id"] },
...
```
  ...
  <LanguageTabContent value="python">
```python filename="LifecycleMV.py" copy
from moose_lib import MaterializedView, MaterializedViewOptions, LifeCycle

# DELETION_PROTECTED: Moose will auto-create but never drop or update
protected_mv = MaterializedView[TargetSchema](MaterializedViewOptions(
    select_statement="SELECT id, value FROM {source_table}",
    select_tables=[source_table],
    table_name="protected_target",
    order_by_fields=["id"],
    materialized_view_name="protected_mv",
    life_cycle=LifeCycle.DELETION_PROTECTED,
))

# EXTERNALLY_MANAGED: Moose will not touch this MV at all
external_mv = MaterializedView[TargetSchema](MaterializedViewOptions(
    select_statement="SELECT id, value FROM {source_table}",
    select_tables=[source_table],
    table_name="external_target",
    order_by_fields=["id"],
    materialized_view_name="external_mv",
...
<Tabs>
  <TabsList>
    <TabsTrigger value="option1">Option 1: Inline</TabsTrigger>
    <TabsTrigger value="option2">Option 2: Decoupled</TabsTrigger>
  </TabsList>
...
new MaterializedView<Dest>({
  selectStatement: sql.statement`SELECT id, toInt32(value) AS value FROM ${sourceTable}`,
  selectTables: [sourceTable],
...
<Tabs>
  <TabsList>
    <TabsTrigger value="simple">Simple Aggregations</TabsTrigger>
    <TabsTrigger value="complex">Complex Aggregations</TabsTrigger>
  </TabsList>
...
const webStmt = sql.statement`SELECT toDate(ts) AS day, user_id, 1 AS events FROM ${webEvents}`;
...
const mobileStmt = sql.statement`SELECT toDate(ts) AS day, user_id, 1 AS events FROM ${mobileEvents}`;
...
<Callout type="info" title="Recommended: ClickHouse SQL Reference" icon="Database" href="https://clickhouse.com/docs/en/sql-reference" ctaLabel="Learn more" compact>
...
const transformation = sql.statement`
  SELECT
...
In dev mode, when a MaterializedView is created or updated, Moose automatically backfills the destination table by running your `SELECT`.

In production, backfill is not automatic to avoid duplicate data if new rows arrive during the backfill. See [Backfill new Materialized Views](/guides/performant-dashboards#backfill-new-materialized-views) for details.
...
You can see the SQL that Moose will run to backfill the destination table when you generate the [Migration Plan](/moosestack/olap/planned-migrations).
...
<Tabs>
  <TabsList>
    <TabsTrigger value="referencing">Referencing the target table</TabsTrigger>
    <TabsTrigger value="decoupled">Decoupled target tables</TabsTrigger>
  </TabsList>
...
const autoFinalized = sql.statement`
