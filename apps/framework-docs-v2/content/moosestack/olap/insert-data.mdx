...
In **TypeScript**, use the `Insertable<T>` utility type for type-safe inserts that omit computed fields and make DEFAULT fields optional. In **Python**, Pydantic models with `ClickHouseAlias` or `ClickHouseMaterialized` annotations automatically treat those fields as optional at construction time.
</Callout>
