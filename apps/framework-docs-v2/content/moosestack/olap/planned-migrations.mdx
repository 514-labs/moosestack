...
2. **Destructive operations** — If the plan contains table drops, column removals, table recreates, or view removals, Moose shows a `DANGER` banner and asks for confirmation. If you reject, Moose prints guidance on how to create a versioned table instead (e.g., `Events` → `Events_v2`) and aborts.

3. **Backfill for versioned tables** — When creating a new versioned table (e.g., `Events_v2`) whose schema matches the existing base table, Moose offers to append an `INSERT INTO ... SELECT` backfill operation to the plan.
