...
By default, Moose uses `docker` as the container runtime for local development infrastructure. You can switch to an alternative Docker-compatible CLI such as [Finch](https://github.com/runfinch/finch), [Podman](https://podman.io/), or [nerdctl](https://github.com/containerd/nerdctl).
...
|:----|:-------------|:--------|:------------|
| `container_cli_path` | `MOOSE_DEV__CONTAINER_CLI_PATH` | `docker` | Container CLI executable name or path. Must support `compose`, `ps`, `exec`, `logs`, and `info` subcommands. |

<Callout type="info" title="Finch Setup">
[Finch](https://github.com/runfinch/finch) is an open-source container tool by AWS that provides a Docker-compatible CLI backed by containerd and nerdctl.
