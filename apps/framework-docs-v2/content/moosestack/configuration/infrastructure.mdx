...
This reference covers the configuration for the core infrastructure components that Moose manages or connects to.
...
See the [Environment Variables](/moosestack/configuration/environment-variables) guide for details on managing secrets.
</Callout>
...
<Callout type="info" title="Dev vs. Prod">
In `moose dev`, these settings configure the ClickHouse container created by Moose. In production, they define how Moose connects to your managed ClickHouse instance.
</Callout>
...
| `host_port` | `MOOSE_CLICKHOUSE_CONFIG__HOST_PORT` | 18123 | HTTP interface port. |
...
| Key | Env Variable | Default | Description |
|:----|:-------------|:--------|:------------|
| `broker` | `MOOSE_REDPANDA_CONFIG__BROKER` | "localhost:19092" | Kafka broker address. |
...
| Key | Env Variable | Default | Description |
|:----|:-------------|:--------|:------------|
| `url` | `MOOSE_REDIS_CONFIG__URL` | "redis://127.0.0.1:6379" | Full connection URL (can include password). |
