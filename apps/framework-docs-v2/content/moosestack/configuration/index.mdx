...
Moose settings live in two places: a project config file and runtime environment variables. The config file is tracked in git and defines the shape of your app; env vars are per-environment values (often secrets) that override the file at runtime. Moose loads both at startup, with env vars winning when they overlap.
...
Moose resolves configuration values in the following order (highest priority first):
...
Moose automatically loads `.env` files in order of precedence. Files loaded later override values from earlier files.
