...
2. **Background Merge**: Rows with the same ORDER BY key are combined—numeric columns are summed, non-numeric columns keep one value.
3. **Query**: Read summed results directly (duplicates may exist until merges complete).
...
Moose does not define its own merge behavior—this engine behaves according to ClickHouse’s `SummingMergeTree` implementation.
