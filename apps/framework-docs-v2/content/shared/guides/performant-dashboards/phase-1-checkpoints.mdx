...
4. Choose requests that cover a reasonable recent time window (e.g. last 7–30 days). You will seed your local ClickHouse to match this window after identifying the source tables in Checkpoint 2.
...
##### Checkpoint 3 — Seed your local ClickHouse database

Goal: seed your local ClickHouse with enough production data to fully cover the test cases from Checkpoint 1. You now know the source tables (from Checkpoint 2) and the time windows and filter values (from Checkpoint 1 test cases).

Prerequisites:
- Your dev server is running (`moose dev`)
- Export your production ClickHouse HTTPS connection string:

```shell
export CLICKHOUSE_PROD_URL="<YOUR_HTTPS_CONNECTION_STRING>"
```

Your connection string should start with `https://` (for example, `https://username:password@host:8443/database`).

Steps:

1. For each source table identified in Checkpoint 2, seed the rows that cover your test case time windows:
...
2. Verify the seeded data covers your test cases:
...
```ts filename="moosestack/app/queries/order-fulfillment-olap-translation.ts" copy
