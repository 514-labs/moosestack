...
3. Before writing ClickHouse SQL, extract and document the OLTP query semantics in `context-map.md` (source tables + joins, filters, parameter defaults, group-by/aggregations, and edge cases).
4. Record ambiguities and assumptions as an inline comment in the ClickHouse translation file.
5. Do not fabricate test case data, expected outputs, or verification evidence.
...
- Updated `context-map.md` Phase 1 Notes with OLTP query semantics (tables, joins, filters, parameter defaults, aggregations, edge cases).
...
For components that read `EXTERNALLY_MANAGED` tables, local data coverage is controlled in `moose.config.toml` via `[dev.remote_clickhouse]` and `[dev.externally_managed.tables]` (especially `sample_size`) rather than a Phase 1 seeding step.
...
2. Create `moosestack/app/queries/<COMPONENT_NAME>-olap-translation.ts`.
