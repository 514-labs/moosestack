...
You'll set up the foundation for the rest of this guide. You'll initialize a MooseStack project in your monorepo and deploy your new project to [Boreal](https://boreal.cloud) for preview deployments and migration planning.
...
<GuideStepper.WhatYouGet>
  - `moose` available in your terminal - Local CLI ready for project
...
<ConditionalContent whenId="source-database" whenValue="postgres">  - Your ClickHouse Cloud HTTPS connection string</ConditionalContent>
</GuideStepper.WhatYouNeed>
...
ClickPipes owns these replicated tables, so MooseStack does not manage their lifecycle. This is reflected the `OlapTable.lifeCycle: LifeCycle.EXTERNALLY_MANAGED` property in all introspected models in `app/externalModels.ts`.
