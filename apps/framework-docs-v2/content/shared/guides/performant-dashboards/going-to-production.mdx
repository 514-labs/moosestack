import { GuideStepper } from "@/components/mdx/interactive/guide-stepper";
...
<GuideStepper.Step id="going-to-production-step-1" number={1} title="Generate a migration plan" summary="Review which tables and views will be created, modified, or deleted before shipping to production.">
<GuideStepper.AtAGlance title="Confirm that your changes are ready to ship">
...
<GuideStepper.Step id="going-to-production-step-2" number={2} title="Review the migration plan">
<GuideStepper.AtAGlance title="How to review the migration plan">
...
<GuideStepper.AtAGlance title="At this point you should have">
...
<GuideStepper.AtAGlance title="Prerequisite: Staging Database Connection String">
Before you can inspect the preview environment’s database state, you need to get the database HTTPS connection string from Boreal. Use the same steps you followed earlier to get the connection string.

Importantly, make sure the Boreal dashboard is set to your feature branch, not `main`. You can confirm this by checking the branch selector in the left sidebar of the project dashboard.
</GuideStepper.AtAGlance>
...
<GuideStepper.Checkpoint title="Merge the PR to deploy to production">
...
<GuideStepper.AtAGlance title="Only backfill new materialized views">
