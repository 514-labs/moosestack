...
- `BOREAL_HOST` is the host for your production deployment in Fiveonefour hosting. Copy it from the URL in your project overview dashboard:
  {/* image */}
- `BOREAL_ADMIN_API_BEARER_TOKEN` is sent in the request header when calling the Fiveonefour hosting Admin API at `BOREAL_HOST`. This is the API key. It is a secret and must not be committed to source control. Store it securely in a password manager.
