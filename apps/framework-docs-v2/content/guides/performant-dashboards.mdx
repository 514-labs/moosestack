...
For this guide, your source database (Postgres or SQL Server) remains the system of record; your application continues writing to it as usual. You’ll add CDC (ClickPipes or Debezium) to stream changes into ClickHouse in real time. Think of ClickHouse as a read-optimized “replica” for analytics: it’s fed by CDC, and you’ll build query-optimized views/materializations on top of it (not run OLTP transactions on it) to serve your dashboard queries. You'll use MooseStack and Boreal to build this analytics layer on ClickHouse and serve your dashboard queries.

By the end of this guide you’ll have:

1. A real-time replication pipeline feeding ClickHouse from your source database.
2. A MooseStack project that defines the ClickHouse analytics layer (views, materialized views, serving tables) and the handler functions your dashboard API endpoints will run to query these tables.
3. A Boreal-backed deploy workflow with preview environments and migration plans so you can validate changes before merging to `main`.

Use the selectors below to tailor the steps to your environment.
...
1. Set up ClickHouse Cloud **ClickPipes** to replicate your Postgres tables into ClickHouse.
2. Add a **MooseStack** project to your repo and run `moose db pull` to generate typed models for the ClickPipes-owned tables.
3. Connect **Boreal** so pull requests get preview environments and migration plans.
4. Build ClickHouse-native resources in MooseStack (views, materialized views, serving tables) and wire up your dashboard queries.
5. Validate in preview, then merge to apply the reviewed migrations to your main ClickHouse database.
</ConditionalContent>

<ConditionalContent whenId="source-database" whenValue="sqlserver">
1. Add a **MooseStack** project to your repo (you’ll define the Debezium ingest pipeline in this project).
2. Connect **Boreal** so pull requests get preview environments and migration plans.
3. Prepare your local dev environment to work with ClickHouse.
4. Set up **Debezium** to stream SQL Server changes into ClickHouse via your MooseStack ingest API.
5. Build ClickHouse-native resources in MooseStack (views, materialized views, serving tables) and wire up your dashboard queries.
6. Validate in preview, then merge to apply the reviewed migrations to your main ClickHouse database.
</ConditionalContent>
...
`moose db pull` writes a file at `app/clickpipesExternalModels.ts`. Load it once from `app/index.ts` so Moose can discover those table definitions (we use an export here so other code can import these tables too):
...
To make this repeatable, add a script to `moosestack/package.json`:
...
Copy the code below into `moosestack/src/cdc/DebeziumChangeEvent.model.ts`. It defines the `DebeziumChangeEvent` payload shape and wires up an `IngestApi` + `Stream` with a Dead Letter Queue (DLQ) so failed events are captured for inspection.

```ts filename="moosestack/src/cdc/DebeziumChangeEvent.model.ts" copy
...
```typescript filename="moosestack/src/cdc/tableStreamMap.ts" copy
...
#### Step 0: Seed your local ClickHouse database
...
First, export the **HTTPS** ClickHouse connection string from the **previous Boreal step** (the ClickHouse connection details Boreal shows you) as an environment variable:

```shell
export CLICKHOUSE_PROD_URL="<YOUR_HTTPS_CONNECTION_STRING>"
```

**Important:** `moose seed clickhouse` uses ClickHouse's HTTPS protocol. Your connection string should start with `https://` (for example, `https://username:password@host:8443/database`).

Then run the [`moose seed` command](https://docs.fiveonefour.com/moosestack/moose-cli?lang=typescript#seed-clickhouse) to pull data from your production ClickHouse database:

```shell
moose seed clickhouse --connection-string "$CLICKHOUSE_PROD_URL" --limit 1000
```
...
```shell
moose seed clickhouse --connection-string $CLICKHOUSE_PROD_URL --all
```
...
Assumptions:
- `CLICKHOUSE_PROD_URL` is set and looks like: `https://default:PASSWORD@HOST:8443/DB_NAME`
