...
Your source database (Postgres or SQL Server) remains your system of record, and your application continues writing to it as usual. CDC (ClickPipes or Debezium) streams changes into ClickHouse in real time so dashboards can read from ClickHouse instead of competing with OLTP workloads.
...
If you’re building a new dashboard or data-connected chat (not migrating an existing dashboard), start with [Chat in Your App](/guides/chat-in-your-app) or [Startup Metrics Dashboard](/guides/data-warehousing/startup-metrics).
</Callout>
...
<Callout type="info" title="Not using ClickPipes?">
...
```ts
// File: moosestack/app/models/OrderFulfillmentServing.model.ts
...
```ts
// File: moosestack/app/views/OrderFulfillmentMV.view.ts
