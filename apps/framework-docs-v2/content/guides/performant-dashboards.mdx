...
* Running analytical queries (aggregations, wide scans, complex joins) on your transactional database degrades performance and reliability.
...
The root cause is usually analytical queries (aggregations, wide table scans, complex joins) running on your existing transactional database, which compete with the core transactional workloads that power the rest of your application.

This guide lays out an step-by-step path to offload those analytical workloads to a purpose-built analytical database (ClickHouse), that can be done incrementally and without needing to rearchitect your existing application.
...
While its common to delay adopting OLAP, there's an inflection point at which it becomes a real business risk. OLAP migrations are non-trivial and typically take 3-6 months with traditional data engineering tooling. If you wait too long to start and/or choose tooling with a steep learning curve, you may not move fast enough to fix the problem before users start churning. 

### When to pull the trigger

You'll know you've hit the inflection point when warning signs start to appear in your day-to-day product and engineering work.

Users want more metrics, filters, breakdowns, or longer time ranges in their dashboards, but engineering can't deliver these seemingly small changes because they could jeopardize the core transactional workloads the rest of your product depends on. Eventually, customers start to complain about load times, Before long, those complaints surface in retention metrics, support tickets, and sales calls.
