...
>
  <CustomizeGrid columns={1}>
    <SelectField
      ...
      options={[
        { value: "debezium", label: "SQL Server (Debezium)" },
        { value: "clickpipes", label: "Postgres (ClickPipes)" },
      ]}
...
<ConditionalContent whenId="cdc-method" whenValue="clickpipes">
### ClickHouse Cloud ClickPipes
This section is coming soon. For now, use ClickHouse Cloud **ClickPipes** to stream your Postgres data into ClickHouse, then continue with the rest of this guide once the ClickHouse tables are present.

<Callout type="info" title="Tip">
If you select ClickPipes, make sure your pipeline writes into the same ClickHouse database you configured in Boreal (so `moose seed clickhouse` and your preview environments target the same schema).
</Callout>
...
<ConditionalContent whenId="cdc-method" whenValue="debezium">
