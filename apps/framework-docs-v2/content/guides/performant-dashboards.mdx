...
* Running analytical queries (aggregations, wide scans, complex joins) on your transactional database degrades performance and reliability. Users wait for data, your core app slows, and you risk downtime during peak usage.
* Moving analytics to ClickHouse incrementally yields 10–100x faster dashboards (10x for simple aggregations, 50-100x for complex joins and time-series queries). Complex reports load in under a second without rewriting your frontend.
...
<BulletPointsCard
  ...
  bullets={[
    ...
    },
    {
      ...
      description: "Updates are risky enough to stay stuck in the backlog, so analytics features stagnate."
    },
    {
...
The root cause is usually analytical queries running on your transactional database. These queries—aggregations, wide table scans, complex joins—compete with the transactional workloads that power the rest of your application.

This guide lays out a step-by-step path to move those analytical workloads to OLAP, **without redesigning your product or rewriting your frontend**.
...
Most teams don't start with OLAP. Delaying is often the right call early on:
  ...
  bullets={[
    {
      ...
      description: "When you're under 1M rows or serving <100 concurrent users, the transactional workload is mission-critical and easiest to justify optimizing."
    },
    {
...
This is why it's common to delay adopting OLAP until it becomes a real business risk. The issue is timing. OLAP migrations are non-trivial and typically take 3-6 months with traditional data engineering tooling. If you wait too long or choose tooling with a steep learning curve, you may not move fast enough to fix the problem before users start churning.

### When to pull the trigger

Consider this shift once warning signs appear in your day-to-day product and engineering work.

It usually starts small: dashboards feel a bit laggy, then noticeably slow as data grows. Users want more filters, breakdowns, or longer time ranges. But engineering can't safely ship these improvements—even small changes could jeopardize the core transactional workloads the rest of your product depends on.

Before long, customers complain about load times. Those complaints surface in retention metrics, support tickets, and sales calls.
...
This guide outlines an incremental path for delivering 10–100x faster dashboard load times without putting your core product at risk.

The core idea: move expensive analytical queries from your existing transactional database to a purpose-built analytical database (ClickHouse) without changing your existing frontend UI or business logic.

This approach fits your existing engineering workflow. Changes are TypeScript, live in the same codebase, and move through your normal review and release process—which means your team doesn't need to learn new tools or hand off ownership to a separate data team. Everything can be validated in staging environments before shipping to production, so you can move fast without breaking things.

Compared to a full-stack refactor or a data-team-led migration, this approach ships your first migrated dashboard in 2-4 weeks (vs 3-6 months), carries minimal risk to your production systems, and lets you scale analytics incrementally as your data grows.

### What success looks like

When this works, the impact is immediate for both product and engineering.

As data volume and usage grow, dashboards stay fast and predictable instead of degrading over time. Analytical workloads no longer compete with core transactional traffic, so the rest of your application runs more reliably.

Engineering can ship new dashboards, metrics, and breakdowns as routine product work—not risky, backlog-bound projects. This unblocks your product roadmap: the analytics features that used to take 2-4 weeks or sit in backlog limbo now ship in 1-3 days, letting you respond to customer requests and competitive pressure faster.
...
**AI copilot (optional but recommended):** We use an AI copilot to accelerate the migration—it handles complex query translations and helps you iterate faster. However, you can complete every step manually if you prefer. The examples use Visual Studio Code [configured with Github Copilot](https://code.visualstudio.com/docs/copilot/setup), but any AI-enabled editor (Claude Code, Cursor, Windsurf, etc.) will work. Without an AI copilot, expect to spend more time writing and debugging SQL manually.
...
Before installing MooseStack, ensure you have:

* **Node.js 20+**: MooseStack requires Node.js version 20 or higher. [Download here](https://nodejs.org/en/download) if you don't have it installed. Check your version with `node --version`.

* **Docker Desktop**: MooseStack uses Docker to run ClickHouse, Redpanda, and other infrastructure locally. [Download Docker Desktop](https://docs.docker.com/desktop/) if you don't have it. After installing, make sure Docker Desktop is running before proceeding.

* **MacOS, Linux, or WSL 2+ on Windows**: MooseStack runs on Unix-based systems.
  - **Windows users**: You'll need WSL 2 (Windows Subsystem for Linux). [See the appendix](#wsl-setting-up-a-wsl2-linux-instance-to-run-moosestack-on-windows) for complete WSL 2 setup instructions if you don't have it configured yet.
  - **MacOS/Linux users**: You're all set—skip to the next step.
...
1. **[Moose Dev MCP server](https://docs.fiveonefour.com/moosestack/moosedev-mcp?lang=typescript)**: Lets your AI copilot query your local ClickHouse database, inspect logs, and explore infrastructure using natural language. Your copilot can verify query results, debug issues, and validate migrations without you copying/pasting commands.
2. **[Language Server (LSP) for MooseStack TypeScript](https://github.com/514-labs/moosestack-lsp)**: Validates SQL syntax within your MooseStack code in real-time, catching errors before you run queries. (Coming soon, see appendix).
...
This section walks you through setting up CDC using Debezium.

Before you start, gather the following credentials for your MSSQL database:
...
First, make sure your MSSQL Server is properly configured to work with Debezium.

**Step 0: PowerShell SQL query command**

You'll use this PowerShell command pattern throughout this guide to execute SQL commands against your MSSQL database.
...
**⚠️ Critical:** SQL Server Agent MUST be running for CDC to capture changes!
...
In this section, you'll select a dashboard or report component to migrate, then:

* Create a development branch (also used by Boreal for preview environments)
* Set up CDC for the supporting tables (guide coming soon)
* Set up local context for development:
  * Seed your local environment from production
  * Bring in other necessary context
* Create, iterate on, and test the migrated dashboard elements in MooseStack

#### Create a branch

Branches manage code versioning and spin up "preview branches" in Boreal.
...
To validate that your migrated queries return correct results, you need realistic production data locally. The [`moose seed` command](https://docs.fiveonefour.com/moosestack/moose-cli?lang=typescript#seed-clickhouse) pulls data from your production ClickHouse database so you can test against real data patterns, edge cases, and volumes.
...
You can adjust the `--limit` value based on how much data you need for testing. Start with 100-1000 rows to keep things fast. If you need your full production dataset locally, you can copy all data using `--all` (note: this can take hours for large datasets and requires disk space equivalent to your production database size):
...
In this section, you'll migrate an existing OLTP-backed dashboard or report to an OLAP-backed implementation (ClickHouse + MooseStack) **without changing how the frontend talks to the backend**.
...
You’ll implement **new OLAP query handlers** in your MooseStack project—one per dashboard component/report you migrate. Then, in your existing application backend, you’ll repoint the endpoint to call the new OLAP handler instead of the original OLTP implementation.

AI copilots excel at this type of migration because they can translate complex OLTP queries (often stored procedures with nested CTEs and joins) into equivalent ClickHouse SQL while preserving exact output. You provide the copilot with your existing query logic and example request/response pairs ("golden cases"), it generates the ClickHouse translation and optimization plan, and you verify correctness at each step. This gives you AI speed without scattering one-off SQL strings across your codebase.

This guide follows a three-phase migration pattern:

1. **Parity (correctness)**: Reproduce existing API behavior on ClickHouse by translating your OLTP query logic into ClickHouse SQL. This validates correctness before optimizing, so you can ship a working replacement immediately if needed.
2. **Optimize (OLAP best practices)**: Move expensive operations (joins, reshaping, stored-procedure-like logic) from query time (when users request data) to write time (when data arrives) using Materialized Views. This is where you unlock the 10-100x performance gains—dashboards that took 10-15 seconds now return in 100-500ms.
3. **Serve (stable interface)**: Point your Query Layer at the pre-computed serving tables, so API handlers stay thin and dashboards stay fast. This makes future changes easy—adding filters, breakdowns, or new metrics becomes routine product work instead of risky database rewrites.  


#### Choose the component you want to migrate

Pick a specific dashboard component or report to migrate. You’ll create one context pack per component.
...
This workflow uses the starter kit in `examples/dashboard-migration/` to keep your "context pack" and prompt consistent across migrations. Every component migration follows the same process, the copilot gets all the information it needs upfront, and you avoid repetitive back-and-forth.
...
- **Current OLTP implementation (what you’re replacing)**: links/file paths to the frontend caller + backend handler, plus any supporting SQL artifacts it relies on (tables/views/stored procedures)
...
**Important:** don’t paste the prompt until the context pack is filled out. The copilot should treat `@context/migrations/COMPONENT_NAME.md` as the source of truth.
...
#### Phase 1: Parity translation (OLTP query logic → ClickHouse SQL)
...
The priority is **correctness**, not OLAP optimization. Getting a working, correct translation first means you have a rollback-ready implementation if you need to ship urgently—and it gives you a solid foundation for optimization without risking correctness.

**You do**

- Run the existing system for a set of golden requests and capture the expected responses (your “golden cases”).
- Give your copilot the right context (contract + OLTP handler/SQL + golden cases) so it can generate the parity SQL.
- Run the copilot’s generated ClickHouse SQL against your test data (typically a local ClickHouse seeded via `moose seed`) and compare the output to your golden cases.
...
At this point, you likely have a parity query that’s correct—but still too expensive to run on every dashboard request.

That's because OLTP query logic typically does all the heavy lifting at query time—joins, CTE chains, and stored-procedure-like reshaping all happen when the user requests data. A 1:1 ClickHouse translation usually preserves that pattern, like this:
...
In OLAP, this pattern is inefficient: every dashboard refresh forces ClickHouse to redo the same joins, reshaping, and aggregations from scratch—even when users are only changing filters, sorts, or time ranges.

ClickHouse best practices avoid stored-procedure-like pipelines at read time, and instead shift the reusable work to write time (using Materialized Views) whenever possible. See ClickHouse’s guidance here: [`Stored procedures and query parameters in ClickHouse`](https://clickhouse.com/docs/guides/developer/stored-procedures-and-prepared-statements).

So Phase 2 is about turning the parity query into a **serving-ready table**: promote the reusable joins/reshaping/derived fields into a [Materialized View](/moosestack/olap/model-materialized-view), then keep request-time logic focused on slicing/sorting/pagination over that serving table. This shift from "compute on read" to "compute on write" is where the performance gains come from.

**You do**

- Identify the expensive operations in the parity query (joins, layered CTEs, stored-procedure-like pipelines).
- Prompt your copilot to propose which operations should move to write time ([Materialized Views](/moosestack/olap/model-materialized-view)) versus which should stay flexible at query time.
...
- The serving query is simpler (50-90% fewer lines of SQL, 0-2 joins instead of 5+) and 10-100x faster.
...
In MooseStack, Materialized Views precompute joins, aggregations, and derived fields as data arrives, persisting results in a physical table. Your API handlers query these precomputed tables instead of executing complex queries at request time. Dashboard performance stays fast and predictable regardless of filters, breakdowns, or time ranges. Once the MV is in place, adding new dashboard features becomes trivial because the expensive work is already done.
...
- **Data is flowing**: new inserts populate the serving table (or you've run a backfill if you need historical parity).
- **Row counts look sane**: use `moose query` to compare counts between source tables and the serving table for a known time window (expect exact match or document any known discrepancies).
- **Spot-check results**: run 3-5 representative queries against the serving table and confirm key fields/aggregates match your expectations.
- **Golden cases still pass**: update your handler to read from the serving table (even with temporary SQL) and confirm all golden test responses match end-to-end.

Once those checks pass, you’re ready for the next step: point your Query Layer `QueryModel` at the serving table so the handler stays thin.
...
1. **Dynamic raw SQL string composition in each API handler**: Build the SQL in each API handler at request time (dynamic `WHERE` / `GROUP BY` / `ORDER BY` / pagination) over the serving table. This works but means repetitive SQL code and higher risk of SQL injection or query errors.
2. **Use the [Query Layer](https://github.com/514-labs/query-layer)** (recommended): Define a typed model once for how the serving table should be queried, then reuse the same query-building logic across handlers. This eliminates repetitive SQL string composition, provides type safety, and makes adding new filters or metrics trivial—just update the model, and all endpoints get the new capability automatically.
...
With your local dev server running, you can connect your frontend application to your new analytical queries using token authentication. This lets you test the complete user experience—dashboards, filters, error states—before deploying to production.
...
✅ Basic request with default parameters (limit 10, offset 0, ascending)
✅ Request with limit 5
✅ Request with offset (pagination - offset 10)
✅ Request with descending order
✅ Request with cacheId
✅ Request with security filter (merchant)
✅ Request with parameter.datasource parameter
✅ Request with orderby leading space (edge case)
✅ Exact structure validation (validates response structure matches document)
✅ Exact value validation (validates first 10 records match document exactly)
```
...
2. Joins `merchants` → `merchant_properties` → `order_metrics`
...
- Join: `merchants` → `merchant_properties` → `order_metrics`
