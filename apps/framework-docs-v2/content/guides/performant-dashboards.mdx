...
1. Setting up ClickHouse Cloud ClickPipes (Postgres CDC) so the replicated tables exist in ClickHouse
2. Creating a MooseStack project and pulling the ClickPipes tables into code (`moose db pull`)
3. Deploying to Boreal for preview branches and migration plans
4. Preparing your local dev environment to work with ClickHouse
5. Migrating one dashboard component from OLTP → ClickHouse
6. Shipping to production safely with a preview environment first
</ConditionalContent>

<ConditionalContent whenId="source-database" whenValue="sqlserver">
1. Adding MooseStack to your project
2. Deploying to Boreal for preview branches and migration plans
3. Preparing your local dev environment to work with ClickHouse
4. Setting up real-time data replication (CDC) with Debezium
5. Migrating one dashboard component from OLTP → ClickHouse
6. Shipping to production safely with a preview environment first
</ConditionalContent>

For this guide, assume a **remote ClickHouse instance** (for example, ClickHouse Cloud) and a **local MooseStack project** in your repo that you’ll use for development.
...
ClickPipes mirrors changes from Postgres into ClickHouse in real time, so your dashboards always reflect the latest state without custom sync jobs or batch ETL scripts.

Set up ClickPipes first so the replicated tables exist in ClickHouse. In the next step, you’ll initialize a MooseStack project (`typescript-empty`) and run `moose db pull` to generate external models for those tables.

Tip: by default `db pull` writes to `app/externalModels.ts`. For ClickPipes, we recommend using `--file-path` to name the file something self-explanatory like `app/clickpipesExternalModels.ts`.
...
ClickPipes creates and manages the replicated raw tables in ClickHouse. MooseStack reads from these tables and creates its own ClickHouse resources (views, materialized views, serving tables) alongside them—MooseStack does not rewrite or manage ClickPipes-owned tables.
</Callout>

Once the pipeline is running and you can see replicated tables in ClickHouse Cloud, continue to [**Add MooseStack to your Project**](#add-moosestack-to-your-project).
</ConditionalContent>
