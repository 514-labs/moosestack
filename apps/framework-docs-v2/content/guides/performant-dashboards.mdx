...
MooseStack is where you define that ClickHouse analytics layer **in code**. In local development, you’ll iterate on schemas (views, materialized views, serving tables) that read from the CDC-replicated tables, and run the same queries/endpoints your dashboard will call.
