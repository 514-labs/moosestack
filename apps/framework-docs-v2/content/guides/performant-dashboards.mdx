...
That’s because OLTP query logic often relies on “read-time orchestration” (just-in-time joins, layered CTE chains, stored-procedure-like reshaping). A 1:1 ClickHouse translation usually preserves that shape, like this:
...
In OLAP, this “read-time orchestration” is an antipattern: every dashboard refresh forces ClickHouse to redo the same joins, reshaping, and aggregations from scratch—even when users are only changing filters, sorts, or time ranges.
