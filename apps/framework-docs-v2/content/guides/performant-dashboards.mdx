...
<CopilotCallout
  ...
  systemPrompt="Goal: translate the existing handler/query logic into
ClickHouse SQL that returns byte-for-byte identical JSON to the current
OLTP endpoint using MooseStack.
...
<CopilotCallout
  ...
  systemPrompt="Goal: optimize the provided unoptimized ClickHouse query function by designing a serving table and Materialized View that pre-computes the joins and aggregations at write time.
...
- The unoptimized ClickHouse query from Phase 1
- The MooseStack source table model files referenced in the unoptimized ClickHouse query function
