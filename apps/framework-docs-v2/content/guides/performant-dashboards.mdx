...
import { FileTree, CTACards, CTACard, Callout, BulletPointsCard } from "@/components/mdx";
...
* You can get 10–100x faster dashboards by moving analytics to ClickHouse incrementally—without rewriting your frontend.
...
<CTACards columns={3}>
  ...
  <CTACard
    title="Setup"
    description="Get your local dev environment ready"
    ctaLink="#setting-up-your-development-environment"
    ctaLabel="Prepare"
    Icon="Wrench"
  />
</CTACards>
...
In most cases, the root cause is that you’re running analytical query patterns (group by / aggregations / wide scans / joins) on the same transactional database that powers the rest of your application.
...
LionHeart is where F45’s most engaged members track workout performance and progress. As usage and data volume increased, the original setup—running analytics directly on the core transactional database—became a bottleneck. Reports were shipped to users as static images of charts. There was no interactivity, and improving the experience carried too much risk to justify prioritization. The team wanted dynamic, interactive dashboards for years, but the existing architecture kept the work stuck in the backlog.
...
Normally teams don’t start with OLAP because it’s rational to delay it:
...
<BulletPointsCard
  ...
  bullets={[
    {
      title: "The OLTP path is the highest-ROI path—at first",
    ...
    },
    {
      ...
      description: "OLTP-backed analytics can look “fine” until data volume and concurrency cross a threshold (often ~10M rows)."
    },
    {
...
This is why its common to push OLTP database migrations until it becomes a real business risk. The issue is timing. OLAP migrations are non-trivial and routinely take months. If you wait too long and/or choose tooling with a steep learning curve, you may not move fast enough to fix the problem before users start churning.

### When to pull the trigger

You should seriously consider this shift once the warning signs appear in day-to-day product and engineering work. It usually starts small: dashboards feel a bit laggy, then noticeably slow as data grows. Users want more filters, breakdowns, or longer time ranges, but engineering can’t safely ship improvements because even small changes could jeopardize core transactional workloads the rest of your product depends on. Before long, customers complain about load times, and those complaints surface in retention metrics, support tickets, and sales calls. Throwing more hardware at the problem either isn’t possible or is prohibitively expensive; even when feasible, this approach only buys temporary relief. The underlying issue is certain to resurface.

### What this guide proposes

This guide outlines an incremental path for delivering **10–100x faster** dashboard load times without putting your core product at risk: move expensive analytical queries from your existing, mission critical transactional database to a purpose built analytical database **without changing your existing frontend UI or business logic**.
...
![Docker Desktop](/public/docker-desktop.png)


Navigate to the your MooseStack project directory and run the development server:
...
![Docker Services](/public/docker-services.png)
...
The recommended approach of this guide allows you to keep:
...
To keep migrations both safe and fast, follow this three-phase pattern:

1. **Parity (correctness)**: reproduce existing API behavior on ClickHouse by translating the underlying OLTP query logic into ClickHouse SQL (without query helpers)  
...
- **Stop after Phase 1** until your golden cases match.
- **Stop extending read-time SQL** once parity is proven; switch to MV design.
- **Only introduce Query Layer** after you have clean serving tables (raw or MV-backed).
...
This workflow uses the starter kit in `examples/dashboard-migration/` to keep your “context pack” and prompt consistent across migrations.

#### You do: assemble the context pack
...
- **Contract**: request/response schema, auth/tenancy rules, 2–5 representative example requests  
- **OLTP implementation**: frontend caller + backend handler + any stored procedures/views it relies on  
- **Verification**: golden input→expected output cases, test data access, known edge cases  

#### You do: paste the prompt

Open `context/migrations/prompt.md`, replace placeholders, and paste it into your copilot. Make sure it references your filled file via `@context/migrations/COMPONENT_NAME.md`.
...
#### Copilot does: produces a plan

The prompt should result in a plan that follows the same three-phase pattern: parity SQL → Materialized Views optimization → Query Layer serving.

## Parity translation (OLTP query logic → ClickHouse SQL)

### What’s happening (concept)
...
At this point in the migration, you have a report that is functionally correct but still not where you want it performance-wise. 
While a common pattern in OLTP is to rely on “read-time orchestration”: just-in-time joins, layered CTE chains, and stored-procedure-like logic to reshape data for each request.

This means that when you translate the OLTP query 1:1 to ClickHouse SQL, you will likely end up with a long CTE chain, complex joins, and stored-procedure-like logic, like this:
...
In OLAP, this “read-time orchestration” is an antipattern; every dashboard refresh forces ClickHouse to redo the same joins, reshaping, and aggregations from scratch, even when users are just changing filters, sorts, or time ranges. As usage grows (more users, more refreshes, more slices), you pay that full computation cost repeatedly—leading to spiky latency, poor concurrency, and expensive queries.

ClickHouse’s best practices are designed to avoid procedural, stored-procedure-like pipelines at read time. Instead, you use declarative transforms, materialized views, and external orchestration to keep serving queries simple and predictable. See ClickHouse’s guidance here: [`Stored procedures and query parameters in ClickHouse`](https://clickhouse.com/docs/guides/developer/stored-procedures-and-prepared-statements).

The goal is to turn the query above into a **serving-ready table**: promote the reusable joins/reshaping/derived fields into a [Materialized View](/moosestack/olap/model-materialized-view) (write time), then keep the Query Layer focused on request-driven slicing (filters, breakdowns, sorting, pagination) over that serving table.

The optimization pattern in this guide is:

1. **Move transformations/joins upstream to write time** using Materialized Views (serving-ready tables)  
2. **Use the Query Layer as the final interface** for dashboard slicing/grouping/sorting/pagination over those serving tables  

### You do

- Identify the OLTP “read-time orchestration” patterns in the parity query (joins, layered CTE reshaping, stored-procedure-like pipelines).
...
### Point the Query Layer at the serving table

Once your MV produces a serving-ready table (with joins/reshaping moved upstream), you’re ready to introduce the Query Layer.

The [Moose Query Layer](https://github.com/514-labs/query-layer) is a set of query helpers you copy into your project. It lets you define a `QueryModel`—a stable, type-safe “semantic contract” over your serving table—so your handler stops embedding complex, raw SQL and instead focuses on wiring: parse request → build query → return results.
...
This is the “final layer” pattern: **Materialized Views shape the data**, and the **Query Layer serves it** to the dashboard through a consistent dimensions/metrics/filters API.
