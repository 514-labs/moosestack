...
#### Prerequisites
- SAP HANA database with CDC permissions
- Tables you want to sync (e.g., EKKO, EKPO, LFA1)
- Redis for CDC state tracking
...
python init_cdc.py --recreate-moose-models --tables EKKO,EKPO,LFA1

# Example output:
# Introspecting table EKKO...
# Introspecting table EKPO...
# ✅ Models generated in app/ingest/cdc.py
```
...
```bash
python init_cdc.py --init-cdc --tables EKKO,EKPO,LFA1
...
- **Redis**: Stores sync watermarks for resume capability
...
- Redis memory: ~1MB per 10,000 rows tracked
