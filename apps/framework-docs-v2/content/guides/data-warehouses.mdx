...
- VS Code
- Cursor
- PyCharm or any editor with Python support
...
```bash
moose init ecommerce-warehouse python
...
ecommerce-warehouse/
├── app/
│   ├── __init__.py         # CRITICAL: Export all models here
│   ├── datamodels/         # Data models (Pydantic BaseModel)
...
**Critical Convention:** All pipeline objects (models, views, workflows, APIs) must be exported from `app/__init__.py` for the CLI to discover them. We'll do this as we build.
...
This tutorial uses Python Pydantic models with MooseStack's `OlapTable` pattern to teach dimensional modeling concepts.

#### Create the Models Directory

```bash
mkdir -p app/datamodels
```

#### Define Customer Dimension

Create `app/datamodels/Customer.py`:

```python
# app/datamodels/Customer.py
...
Now add the OlapTable definition to create the ClickHouse table. Add this to the same file:

```python
# app/datamodels/Customer.py (continued)

from moose_lib import OlapTable, OlapConfig, ReplacingMergeTreeEngine

# Create the ClickHouse table
dim_customer = OlapTable[Customer](
    "dim_customer",
    OlapConfig(
        engine=ReplacingMergeTreeEngine(),
        order_by_fields=["customer_key"],
    )
)
```
...
- `OlapTable[Customer]` - Creates a ClickHouse table based on the Customer model
- `ReplacingMergeTreeEngine` - Automatically deduplicates rows with the same key (good for dimensions)
- `order_by_fields` - Determines the sort order and primary key for the table

You'll follow this same pattern for all dimension and fact tables. For dimensions, use `ReplacingMergeTreeEngine()`. For facts, use `MergeTree()`.

#### Define Product Dimension

Create `app/datamodels/Product.py`:

```python
# app/datamodels/Product.py
...
Create `app/datamodels/DateDimension.py`:

```python
# app/datamodels/DateDimension.py
...
class DateDimension(BaseModel):
    # Primary key
    date_key: Key[int]  # Format: YYYYMMDD (e.g., 20240315)
...
> **Technical Note:** Date dimensions use the `numbers()` table function to generate sequences. In our implementation, `numbers(4018)` generates 0-4017, which we add as days to a start date: `toDate('2020-01-01') + INTERVAL number DAY`. This single INSERT creates 11 years of dates (4,018 rows) in milliseconds. The dimension fits entirely in memory (~400KB) and makes date-based queries 10-100x faster by replacing expensive date calculations with simple integer joins.

#### Define Store Dimension

Create `app/datamodels/Store.py`:

```python
# app/datamodels/Store.py
...
class Store(BaseModel):
    ...
    # Store attributes
    store_name: str
    store_type: str  # 'retail', 'outlet', 'online'
...
Create `app/datamodels/OrderItem.py`:

```python
# app/datamodels/OrderItem.py
...
class OrderItem(BaseModel):
    ...
    product_key: int
    order_date_key: int
    ship_date_key: Optional[int] = None
    store_key: int
...
#### Add OlapTable Definitions to Remaining Models

Now add OlapTable definitions to your other model files. Here are the complete examples:

**Product** (add to `app/datamodels/Product.py`):
```python
from moose_lib import OlapTable, OlapConfig, ReplacingMergeTreeEngine

dim_product = OlapTable[Product](
    "dim_product",
    OlapConfig(
        engine=ReplacingMergeTreeEngine(),
        order_by_fields=["product_key"],
    )
)
```
...
**Store** (add to `app/datamodels/Store.py`):
```python
from moose_lib import OlapTable, OlapConfig, ReplacingMergeTreeEngine

dim_store = OlapTable[Store](
    "dim_store",
    OlapConfig(
        engine=ReplacingMergeTreeEngine(),
        order_by_fields=["store_key"],
    )
)
```

**OrderItem** (add to `app/datamodels/OrderItem.py`):
```python
from moose_lib import OlapTable, OlapConfig, MergeTreeEngine

fact_order_items = OlapTable[OrderItem](
    "fact_order_items",
    OlapConfig(
        engine=MergeTreeEngine(),
        order_by_fields=["order_date_key", "customer_key", "product_key"],
    )
)
```

**Note:**
- Fact tables use `MergeTreeEngine()` (no deduplication), while dimension tables use `ReplacingMergeTreeEngine()` (automatic deduplication)
- For fact tables, don't use `Key[int]` on the surrogate key if it's not part of the `order_by_fields`. The order_by fields become the primary key in ClickHouse.

#### Export Models from Index

**Critical Step:** Edit `app/__init__.py` to export all models and tables:

```python
# app/__init__.py

from .datamodels.Customer import Customer, dim_customer
from .datamodels.Product import Product, dim_product
from .datamodels.DateDimension import DateDimension, dim_date
from .datamodels.Store import Store, dim_store
from .datamodels.OrderItem import OrderItem, fact_order_items
...
> **What just happened?** This single query generated 4,018 rows (11 years of dates) using ClickHouse's `numbers()` function. Each row has pre-calculated attributes like `is_weekend`, `quarter`, and `day_name`. This ~400KB table eliminates expensive date calculations from analytical queries, making them 10-100x faster.
...
Create `app/apis/SalesSummary.py`:

```python
# app/apis/SalesSummary.py
...
Export from `app/__init__.py`:

```python
from .apis.SalesSummary import sales_summary_api
...
```bash
curl "http://localhost:4000/consumption/sales_summary?start_date=2024-01-01&end_date=2024-12-31"
```
...
### Python Best Practices for Data Warehouses

Before we wrap up, here are some Python-specific best practices for building production-ready data warehouses.
...
Add `.venv/` to your `.gitignore` to avoid committing the virtual environment.

#### Type Safety with Pydantic

Leverage Pydantic's runtime validation:
...
This catches data quality issues at runtime before they reach ClickHouse.
...
- **pip-tools** - Generates pinned requirements from high-level dependencies
...
def safe_query(client: MooseClient, query: str):
    try:
...
For large CSV files, use pandas with chunking:

```python
import pandas as pd
from clickhouse_driver import Client

def load_large_csv(file_path: str, table_name: str, chunk_size: int = 10000):
    """Load large CSV in chunks to avoid memory issues"""
    client = Client(host='localhost', port=9000, user='panda', password='pandapass')
...
Structure your Python warehouse code:
...
Add unit tests for your data transformations:

```python
# tests/test_transformations.py
import pytest
from app.datamodels.Customer import Customer
...
def test_customer_validation():
    ...
    assert customer.customer_key == 1

    # Invalid customer (missing required field)
    with pytest.raises(ValueError):
        Customer(customer_key=1, email="test@example.com")
```
