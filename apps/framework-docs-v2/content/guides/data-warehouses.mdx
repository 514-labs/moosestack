...
**Note:** SummingMergeTree can't directly store averages. Instead, we store the sum and count, then calculate the average at query time as `sum_unit_price / price_count`.
