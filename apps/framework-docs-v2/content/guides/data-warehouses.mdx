...
# Export order_items with date keys
psql -d ecommerce_production -c "\COPY (
  SELECT
...
#### Create a Scheduled Refresh Workflow

Create `app/workflows/scheduled_refresh.ts`:

```typescript
// app/workflows/scheduled_refresh.ts

import loadCustomerDimension from './load_customer_dimension';
import loadProductDimension from './load_product_dimension';
import loadStoreDimension from './load_store_dimension';
import loadOrderItemsFact from './load_order_items_fact';

export default async function scheduledRefresh() {
  console.log('Starting scheduled warehouse refresh...');

  try {
    // Load dimensions first (in parallel since they're independent)
    await Promise.all([
    ...
    await loadOrderItemsFact();

    console.log('Facts loaded successfully');

    // Log metrics
    const metrics = await collectMetrics();
    console.log('Refresh complete:', metrics);
...
async function collectMetrics() {
  // Query for row counts, data quality checks, etc.
  return {
    customers: 5000,
    products: 500,
    stores: 50,
...
async function sendAlert(error: Error) {
  ...
  console.log('Alert sent:', error.message);
}
```
...
```typescript
export * from "./workflows/scheduled_refresh";
```

#### Configure Schedule

Create `moose.workflows.yaml`:

```yaml
# moose.workflows.yaml
...
```bash
moose workflow run scheduled_refresh
```

Output:

```
Running workflow: scheduled_refresh
Starting scheduled warehouse refresh...
✓ Dimensions loaded successfully
✓ Facts loaded successfully
Refresh complete: { customers: 5000, products: 500, stores: 50, order_items: 75000, duration_seconds: 45 }
```

#### Configure Data Quality Checks

Add data quality validations to your workflow:

```typescript
// app/workflows/data_quality_checks.ts

export async function checkDataQuality() {
  const checks = [];

  // Check 1: No null customer keys in facts
  const nullCustomers = await query(`
    SELECT COUNT(*) as cnt
  ...
  checks.push({
    name: 'null_customer_keys',
    passed: nullCustomers.cnt === 0,
    message: `Found ${nullCustomers.cnt} null customer keys`
  });

  // Check 2: Total amounts should be positive
  const negativeAmounts = await query(`
    SELECT COUNT(*) as cnt
    FROM fact_order_items
    WHERE total_amount < 0
  `);
  ...
  checks.push({
    ...
    passed: negativeAmounts.cnt === 0,
    message: `Found ${negativeAmounts.cnt} negative amounts`
  });

  // Check 3: Today's data exists
  ...
  checks.push({
    name: 'today_data_exists',
    passed: todayRecords.cnt > 0,
    message: `Found ${todayRecords.cnt} records for today`
  });
...
*This section explains the technical concepts behind what you built in Part 1.*
