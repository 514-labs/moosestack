...
✓ Created table: sales_SalesTransaction
...
function validateRecord(record: SalesTransaction, index: number): ValidationError[] {
  ...
  // Date validation using date-fns for strict ISO 8601 parsing
  // Note: parseISO rejects non-ISO formats like "01/15/2025" that Date.parse would accept
  if (record.saleTimestamp) {
    const parsed = parseISO(record.saleTimestamp);
    if (!isValid(parsed)) {
...
1. When you INSERT transactions into `sales_SalesTransaction`, ClickHouse triggers the MV
...
SELECT * FROM local.sales_SalesTransaction
