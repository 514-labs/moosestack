...
**Choose your path:**
- 📊 Business/ROI focus? → Read Part 1, evaluate if this fits your needs
- 💻 Hands-on builder? → Jump to Part 2, start coding immediately
- 🧠 Concepts first? → Read Part 1, build in Parts 2-6, learn theory in Part 7
...
> **Important:** Moose projects use pnpm workspaces. If you don't have pnpm installed, run `npm install -g pnpm` first. Using `npm install` will fail with `EUNSUPPORTEDPROTOCOL` errors.
...
> **Note:** The first time you run `moose dev`, the CLI may auto-upgrade to match the moose-lib version in your package.json. This is normal behavior and ensures version compatibility.
...
> **Critical:** The `IngestPipeline` must include the type parameter (e.g., `IngestPipeline<SalesTransaction>`). Without it, you'll get the error: "Supply the type param T so that the schema is inserted by the compiler plugin."
...
> **Note:** The credentials `panda`/`pandapass` are local development defaults configured by `moose dev`. Never use these in production—configure secure credentials in your deployment environment.
...
| Invalid date format | `parseISO()` + `isValid()` from date-fns | Query filters fail silently |
| Negative amounts | `totalAmount < 0` | Incorrect revenue totals |
| Math errors | `quantity × unitPrice ≠ totalAmount` | Inconsistent reporting |
...
- Always use `GROUP BY` when querying to ensure accurate aggregation results
...
You should see both `sales` and `DailySalesSummary`.

**What Just Happened:**

You created a materialized view that transforms your data pipeline:

1. **Triggers on INSERT** - Every time data lands in `sales`, ClickHouse automatically runs the aggregation query
...
4. **Requires GROUP BY on queries** - Since MVs aggregate per-batch, always use `GROUP BY` when querying to consolidate results correctly
...
async function generatePdf(
  ...
  // Note: createPdfKitDocument is SYNCHRONOUS - do not use await here.
  // It returns a PDFKit.PDFDocument stream object immediately.
...
### Understanding MergeTree and Batch Aggregation

ClickHouse materialized views process data in batches, which has important implications for how you query them.

**How MVs aggregate data:**

1. When you INSERT transactions into `sales`, ClickHouse triggers the MV
2. The MV's SELECT statement runs on the **new batch only**, not all existing data
3. Results are inserted into the target table (`DailySalesSummary`)
4. Multiple batches for the same day/region/category create **multiple rows**

**Example:**

Two separate INSERTs on the same day for West/Electronics:
...
The target table now has **two rows** for the same day/region/category:

```
Row 1: [saleDate=2025-01-15, region=West, category=Electronics, count=50, revenue=5000]
Row 2: [saleDate=2025-01-15, region=West, category=Electronics, count=30, revenue=3000]
```

**This is why you must always use `GROUP BY` when querying:**
...
**Note:** ClickHouse offers specialized engines like `SummingMergeTree` that automatically combine rows during background merges. However, even with these engines, using `GROUP BY` is the safe practice since merges happen asynchronously.
...
- **Batch aggregation** - Why you need GROUP BY when querying MVs
- **Time-series patterns** - How to structure data for efficient time-range queries

These concepts apply beyond this tutorial—they're fundamental to building any high-performance analytics system.
...
Use `pnpm install` instead of `npm install`. Moose uses pnpm workspaces.

**Supply the type param T error:**

Ensure `IngestPipeline` includes the type parameter: `new IngestPipeline<SalesTransaction>(...)`. Without it, the compiler plugin cannot generate the correct schema.

**Primary key must be prefix of sorting key:**

The first field in `orderByFields` becomes the primary key. Don't use `Key<T>` if you want a different ordering.
...
To deduplicate in queries, use `GROUP BY` or subqueries:

```sql
-- Option 1: Aggregate to get unique transactions
SELECT transactionId, max(saleTimestamp) as saleTimestamp, sum(totalAmount) as totalAmount
FROM local.sales
GROUP BY transactionId

-- Option 2: Use a subquery to get the latest record per transaction
SELECT * FROM local.sales
WHERE (transactionId, saleTimestamp) IN (
  SELECT transactionId, max(saleTimestamp)
  FROM local.sales
  GROUP BY transactionId
)
```

**Note:** The `FINAL` modifier only works with specific MergeTree variants (ReplacingMergeTree, CollapsingMergeTree) that support row-level deduplication. Standard MergeTree tables require manual deduplication via queries.
