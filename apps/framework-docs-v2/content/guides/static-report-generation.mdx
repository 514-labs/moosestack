...
This guide walks through automating report generation with MooseStack. You'll build a data pipeline from file ingestion to PDF export, using ClickHouse for fast time-series queries.
...
**Operations teams:** Read Part 1 for the business case, skim Parts 2-6 to see what's involved, then hand off to developers.

**Developers:** Skip to Part 2 and start building. Part 1 has business context if you need it later.

**Data engineers evaluating this approach:** Part 1 covers the business case, Parts 2-6 are the implementation, Part 7 explains the underlying concepts.
...
You're a data engineer at Apex Retail, a chain with 12 stores across 4 regions. Each night, store POS systems export the day's sales as CSV or JSON files.
...
API server starts on http://localhost:4000. Keep this terminal open.
...
If checks fail, wait 20-30 seconds for containers to initialize. First startup downloads Docker images and takes longer.
...
| ClickHouse | 18123 | Columnar database |
...
From one interface, Moose:
1. Inferred the ClickHouse schema
2. Created the table with time-first ordering
3. Set up the Redpanda topic
4. Generated the REST endpoint with validation
...
> **Tip:** In pnpm workspaces, `pnpm exec ts-node` is more reliable than `npx ts-node`. You can also add a script to `package.json`: `"load-file": "ts-node scripts/load-file.ts"` and run `pnpm run load-file sample-data/west-store-001.csv`.
...
What this does:
1. Triggers on INSERT to `sales`
2. Groups transactions by day, collapsing millions of rows into one per day/region/category
3. Stores results in `DailySalesSummary`
4. Requires GROUP BY when querying (MVs aggregate per-batch)
...
100x faster by aggregating at insert time.
...
This creates:
1. REST endpoint at `/consumption/sales-report`
2. Query parameters: `startDate`, `endDate`, optional `region`
3. Queries the materialized view (fast)
4. Parameterized SQL (no injection)
5. Typed JSON response

Single source of truth for report data. PDF generator, dashboards, everything uses this endpoint.
...
> **Note:** This guide uses pdfmake 0.3.x which requires Node.js 20 LTS or higher. The API changed significantly from version 0.2.x—see the [pdfmake 0.3 documentation](https://pdfmake.github.io/docs/0.3/) for details.
...
If the API returns valid JSON, generate the PDF:

```bash
npx ts-node scripts/generate-pdf-report.ts 2025-01-15 2025-01-15 january-15-report.pdf
```

> **Tip:** Always test the API with `curl` before running the PDF generator. The PDF generator has limited error messages if the API fails.
...
Problems: stale data, rescanning, failures leave gaps, scaling complexity.
...
Benefits: always current, no batch infra, scales with writes, failures don't corrupt.
...
|-----------|--------------|
| `SalesTransaction` model | Type-safe schema, auto-provisions ClickHouse |
| File loader | Parses CSV/JSON, sends to ingest API |
| `DailySalesSummary` MV | Pre-aggregates at insert time |
| `sales-report` API | Parameterized date-range queries |
...
**After:** POS uploads to API, ClickHouse aggregates, API serves any date range, PDF generates in <1 second. 24/7. Git history for everything.

**Impact:** 500+ analyst hours/year freed. No delayed reports. Consistent numbers. Instant historical queries. Same code for 12 or 1,200 stores.
...
**CSV date parsing fails:** Use ISO 8601.
