...
If checks fail, wait 10-15 seconds for containers to initialize.
...
Open `january-15-report.pdf`. Contains:
...
**ClickHouse connection refused:** Wait 10-15 seconds for containers. Verify:
...
**Docker not starting:** Run `docker info`. On Mac/Windows, check Docker Desktop is running.

**Model not detected:** Check file saved, export in `app/index.ts`, TypeScript errors in terminal.
...
**Not generating:** Check API accessible, fetch errors in output, pdfmake installed.
