...
description: Clean connector architecture for durable ETL from websocket, API, and database-change sources
...
3. Persist checkpoint only after successful sink write.
4. Clean stop on workflow cancellation.

What it is **not** responsible for:

1. Source protocol details.
2. Source-specific payload mapping.
3. Source-specific destination routing choices.

---

## Connector shape (clear boundaries)

Each connector folder follows the same file roles:

1. `sinks.ts`
- Define Moose destination resources (`OlapTable`, `Stream`).
- Define the `resources` map:
  - `destination` (`Stream` or `OlapTable`)
  - optional `transform(payload, envelope)` when payload is not directly writable

2. `source.ts`
- Connect/auth/subscribe/poll.
- Emit canonical envelopes: `{ resource, payload, checkpoint? }`.
- Handle provider disconnect/error and map to runtime `onDisconnect(...)`.

3. `connector.ts`
- Single composition point.
- Resolve connector env.
- Instantiate `runDurablePipeline(...)`.
- Configure where data is written via `resources`.
- Configure checkpoint persistence.
- Expose long-running workflow (`timeout: "never"`) with cleanup hooks.
- Use `defineConnector(...)` so this composition shape is consistent across sources.

This structure works for websocket feeds, polling APIs, and database change streams.
...
- Pass them to the Moose connector process as environment variables.

3. Reuse existing Supabase generated types:
- Point `supabase.generated.ts` to your shared generated type package.
- Keep source typing and mapping close to connector code.

4. Keep responsibilities separated:
- Source protocol logic in `source.ts`.
- Envelope emission and checkpoint extraction in `source.ts`.
- Destination routing and optional transform in `sinks.ts`.
- Pipeline/workflow instantiation and resource configuration in `connector.ts`.
        ...
        <FileTree.Folder name="app" defaultOpen>
          <FileTree.Folder name="connectors" defaultOpen>
            ...
            <FileTree.Folder name="supabase" defaultOpen>
              <FileTree.File name="sinks.ts" />
              <FileTree.File name="source.ts" />
              <FileTree.File name="connector.ts" />
            </FileTree.Folder>
