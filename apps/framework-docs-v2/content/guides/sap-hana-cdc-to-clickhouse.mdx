...
Copy the SAP HANA CDC pipeline code from the registry to your Moose project:

```bash
# Navigate to your Moose project directory
cd your-moose-project

# Copy the pipeline files from the registry
cp -r path/to/registry/pipeline-registry/sap_hana_cdc_to_clickhouse/* .
```

Alternatively, if using the Moose CLI with registry support:

```bash
moose install pipeline sap_hana_cdc_to_clickhouse
```

This installs:
- `workflows/cdc.py` - CDC workflow for initial load and incremental sync
- `workflows/prune.py` - Cleanup workflow for CDC_CHANGES table
- `init_cdc.py` - Initialization script for generating models and creating triggers
- `connectors/sap_hana_cdc.py` - SAP HANA connection and CDC utilities
...
### 3. Initialize CDC Infrastructure
...
### 4. Start Moose Development Server
