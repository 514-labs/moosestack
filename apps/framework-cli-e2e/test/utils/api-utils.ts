...
/**
 * Verifies BYOF (Bring Your Own Framework) API responses
 * Tests standalone getMooseClients() usage outside of moose dev context
 */
export const verifyByofApi = async (
  endpoint: string,
  ...
  await withRetries(
    async () => {
      ...
      const response = await fetch(
        `${SERVER_CONFIG.url}/api/${endpoint}?${params}`,
      );

      if (!response.ok) {
        const text = await response.text();
        throw new Error(`BYOF API failed with ${response.status}: ${text}`);
      }

      console.log("BYOF API test request sent successfully");
      const json = (await response.json()) as any[];

      if (!Array.isArray(json)) {
        throw new Error("Expected array response from BYOF API");
      }

      if (json.length < 1) {
        throw new Error("Expected at least one item in BYOF API response");
      }

      json.forEach((item: any) => {
        expectedKeys.forEach((key) => {
          if (!item.hasOwnProperty(key)) {
            throw new Error(`Missing property ${key} in BYOF API response`);
          }
        });
      });

      console.log(`BYOF API verification successful: ${json.length} items`);
    },
    {
      attempts: RETRY_CONFIG.API_VERIFICATION_ATTEMPTS,
      delayMs: RETRY_CONFIG.API_VERIFICATION_DELAY_MS,
