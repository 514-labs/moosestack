 ...
 * In alpha mode (no Docker), falls back to the /ready endpoint which verifies
    ...
    try {
      ...
      if (!containerName.trim()) {
        ...
        if (dockerAttempts >= MAX_DOCKER_DETECT_ATTEMPTS) {
          // No Docker container found — assume alpha mode (devkafka embedded)
          log.debug(
            "No Redpanda container found after multiple attempts, using alpha mode readiness check",
          );
          await waitForStreamingAlphaMode(
      ...
      dockerAttempts++;
      if (dockerAttempts >= MAX_DOCKER_DETECT_ATTEMPTS) {
        log.debug("Docker checks failing, using alpha mode readiness check");
        await waitForStreamingAlphaMode(
          timeoutMs - (Date.now() - startTime),
          baseUrl,
          log,
 ...
 */
const waitForStreamingAlphaMode = async (
  remainingMs: number,
  baseUrl: string,
  log: ScopedLogger,
): Promise<void> => {
  ...
  while (Date.now() - startTime < remainingMs - STABILIZATION_DELAY_MS) {
    try {
      ...
      if (response.status === 200) {
        ...
        log.debug("✓ Streaming functions ready (alpha mode)");
        return;
      }
  ...
  throw new Error(
    `Infrastructure did not become ready within the remaining timeout (alpha mode)`,
  );
};
