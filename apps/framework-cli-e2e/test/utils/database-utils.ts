...
// ============ DATE AGGREGATION VERIFICATION UTILITIES ============

/**
 * Verifies Date aggregation data exists in ClickHouse
 */
export const verifyDateAggregationData = async (
  eventId: string,
): Promise<void> => {
  await withRetries(
    async () => {
      ...
      try {
        const result = await client.query({
          query: `SELECT * FROM DateAggregationTest WHERE id = '${eventId}'`,
          format: "JSONEachRow",
        });
        const rows: any[] = await result.json();
        console.log("DateAggregationTest data:", rows);

        if (rows.length === 0) {
          throw new Error(
            `Expected at least one row in DateAggregationTest with id = ${eventId}`,
          );
        }
        ...
        if (row.id !== eventId) {
          throw new Error(
            `id in DateAggregationTest should match the generated UUID`,
          );
        }

        // Verify that Date field exists and is properly formatted
        if (!row.lastUpdated) {
          throw new Error("lastUpdated field should not be null or empty");
        }

        // Verify other required fields
        if (typeof row.value !== "number") {
          throw new Error("value field should be a number");
        }

        if (!row.category || typeof row.category !== "string") {
          throw new Error("category field should be a non-empty string");
        }

        console.log(`Verified DateAggregationTest record: ${eventId}`);
      } finally {
        await client.close();
      }
    },
...
/**
 * Verifies Date aggregation materialized view data
 */
export const verifyDateAggregationSummary = async (
  ...
  await withRetries(
    async () => {
      ...
      try {
        const result = await client.query({
          query: `SELECT * FROM DateAggregationSummary WHERE category = '${expectedCategory}'`,
          format: "JSONEachRow",
        });
        const rows: any[] = await result.json();
        console.log("DateAggregationSummary data:", rows);

        if (rows.length === 0) {
          throw new Error(
            `Expected at least one row in DateAggregationSummary with category = ${expectedCategory}`,
          );
        }
        ...
        if (!row.totalRecords || parseInt(row.totalRecords) < 1) {
          throw new Error("totalRecords should be at least 1");
        }

        if (typeof row.avgValue !== "number") {
          throw new Error("avgValue should be a number");
        }

        // Verify that argMax worked correctly with Date field
        if (!row.mostRecentUpdate) {
          throw new Error(
            "mostRecentUpdate field should not be null (argMax result)",
          );
        }

        console.log(
          `Verified DateAggregationSummary for category: ${expectedCategory}`,
        );
      } finally {
        await client.close();
      }
