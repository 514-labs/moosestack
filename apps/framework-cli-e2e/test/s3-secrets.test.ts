...
const withProcessEnv = (
  env: Partial<Record<keyof typeof S3_TEST_ENV, string | undefined>>,
) => {
  const previousEnv = Object.fromEntries(
    Object.keys(env).map((key) => [key, process.env[key]]),
  ) as Record<keyof typeof S3_TEST_ENV, string | undefined>;
...
if (!SELECTED_LANGUAGE || SELECTED_LANGUAGE === "py") {
  describe("python template tests - S3Queue Runtime Environment Variable Resolution", () => {
    describe("With Environment Variables", () => {
      let devProcess: ChildProcess | null = null;
      let TEST_PROJECT_DIR: string;
      let restoreProcessEnv: (() => void) | null = null;

      before(async function () {
        this.timeout(TIMEOUTS.TEST_SETUP_MS);
        restoreProcessEnv = withProcessEnv(S3_TEST_ENV);
