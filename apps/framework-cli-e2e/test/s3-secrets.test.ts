 ...
 * 5. System shuts down gracefully with clear error messages when env vars are missing
...
describe("S3Queue Runtime Secret Resolution", () => {
  ...
  describe("TypeScript - With Environment Variables", () => {
    ...
    it("should start successfully and resolve environment variables correctly", async function () {
      ...
      // This verifies that:
      // 1. mooseEnvSecrets.get() markers were correctly generated by the TypeScript library
  ...
  describe("TypeScript - Without Environment Variables", () => {
    let devProcess: ChildProcess | null = null;
    let TEST_PROJECT_DIR: string;

    // TODO: This test is currently skipped because the CLI doesn't validate infrastructure
    // at startup - it validates lazily when tables are actually used. We need to add early
    // validation to make this test work.
    it.skip("should shutdown gracefully with clear error message when environment variables are missing", async function () {
      this.timeout(TIMEOUTS.TEST_SETUP_MS);
  ...
  describe("Python - With Environment Variables", () => {
    ...
    it("should start successfully and resolve environment variables correctly", async function () {
      ...
      // If we got here, the server started successfully with the environment variables set
      // This verifies that:
      // 1. moose_env_secrets.get() markers were correctly generated by the Python library
      // 2. The Rust CLI successfully resolved the markers from environment variables
      // 3. The resolved values were used to create the S3Queue table without errors
      //
      // If the environment variables were not resolved correctly, the server would have
  ...
  describe("Python - Without Environment Variables", () => {
    let devProcess: ChildProcess | null = null;
    let TEST_PROJECT_DIR: string;
