...
4. **ClickHouse Best Practices Skill** (optional) — Add it later with the non-destructive `514 agent init` flow instead of rerunning `moose harness init`. Contains rules for schema design, query optimization, insert strategy, and MooseStack-specific patterns.
