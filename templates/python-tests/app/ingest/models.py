...
from moose_lib import Key, IngestPipeline, IngestPipelineConfig, StringToEnumMixin, clickhouse_default, clickhouse_decimal, clickhouse_datetime64, ClickhouseSize
...
# =======Comprehensive Moose-supported Types=========

class CoordinatesNT(BaseModel):
    lat: float
    lng: float
...
class AllTypes(BaseModel):
    ...
    # Signed integers
    int8_col: Annotated[int, "int8"]
    int16_col: Annotated[int, "int16"]
    int32_col: Annotated[int, "int32"]
    int64_col: Annotated[int, "int64"]

    # Unsigned integers
    uint8_col: Annotated[int, "uint8"]
    uint16_col: Annotated[int, "uint16"]
    uint32_col: Annotated[int, "uint32"]
    uint64_col: Annotated[int, "uint64"]

    # Floating points and Decimal
    float32_col: Annotated[float, ClickhouseSize(4)]
    decimal_col: clickhouse_decimal(10, 2)

    # Strings and LowCardinality
    lowcard_col: Annotated[str, "LowCardinality"]
    ...
    # Date and DateTime64
    date_col: date
    dt64_col: clickhouse_datetime64(3)
    ...
    # Map
    map_col: Dict[str, int]

    # Named Tuple
    named_tuple_col: Annotated[CoordinatesNT, "ClickHouseNamedTuple"]

    # JSON
    json_col: Any

    # Nullable/Optional variants (including with ClickHouse defaults)
    optional_uint32: Optional[Annotated[int, "uint32"]] = None
    optional_str_with_default: Optional[Annotated[str, clickhouse_default("''")]] = None
...
all_types_model = IngestPipeline[AllTypes]("AllTypes", IngestPipelineConfig(
    ingest=True,
    stream=True,
    table=True,
    dead_letter_queue=True
