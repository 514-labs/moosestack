...
Live heart rate leaderboard inspired by F45. Python Moose backend with Streamlit UI.

## Dev Server

Two processes:

- **Moose backend:** `moose dev`
- **Streamlit UI:** `streamlit run streamlit_app.py`

Ports used: 4000, 5001, 7233, 8080, 9000, 18123 (Moose), 8501 (Streamlit). See `moose.config.toml` to change.

## Key Files

| File | Purpose |
| --- | --- |
| `app/datamodels/` | Data models (heart rate data) |
| `app/functions/` | Processing functions |
