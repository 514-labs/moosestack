...
A TypeScript MooseStack project with a Next.js frontend (empty).

## Dev Server

Two processes:

- **Moose backend:** `cd moose && moose dev`
- **Next.js frontend:** `cd frontend && npm run dev`

Ports used: 3000 (frontend), 4000, 5001, 7233, 8080, 9000, 18123 (Moose). See `moose/moose.config.toml` to change.

## Key Files

| File | Purpose |
| --- | --- |
| `moose/app/` | Moose backend application code |
| `moose/moose.config.toml` | Moose port and service configuration |
| `frontend/src/app/` | Next.js app directory |
| `frontend/package.json` | Frontend dependencies (React, Next.js) |

## Dev Environment

### Start the dev server

Start the Moose backend first — this starts ClickHouse, the data pipeline, and the MooseDev MCP server on `localhost:4000`.
...
Use `orderByFields` to control ClickHouse table ordering — put your most-filtered columns first. If you have the ClickHouse Best Practices Skill installed, use it to choose the right ordering.
