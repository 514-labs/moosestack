...
describe("formatChatUiErrorMessage", () => {
  it("maps stream parser failures to compatibility guidance", () => {
    expect(formatChatUiErrorMessage("Failed to parse stream response from /api/chat")).toContain(
      "Stream format incompatible.",
    );
  });

  it("preserves other chat error messages", () => {
    expect(formatChatUiErrorMessage("MCP server unavailable")).toBe("MCP server unavailable");
  });
