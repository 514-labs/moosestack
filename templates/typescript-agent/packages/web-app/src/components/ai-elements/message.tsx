...
export const MessageAction = ({
  ...
  const button = (
    <Button aria-label={ariaLabel ?? label} size={size} type="button" variant={variant} {...props}>
      {children}
...
export const MessageBranch = ({
  ...
  const goToPrevious = useCallback(() => {
    const newBranch = safeCurrentBranch > 0 ? safeCurrentBranch - 1 : branches.length - 1;
    handleBranchChange(newBranch);
  }, [safeCurrentBranch, branches.length, handleBranchChange]);

  const goToNext = useCallback(() => {
    const newBranch = safeCurrentBranch < branches.length - 1 ? safeCurrentBranch + 1 : 0;
    handleBranchChange(newBranch);
  }, [safeCurrentBranch, branches.length, handleBranchChange]);
