...
A brainwave capture and analysis system with Moose backend and data analysis service.

## Dev Server

Start with: `cd apps/brainmoose && moose dev`

Ports used: 4000, 5001, 7233, 8080, 9000, 18123. See `apps/brainmoose/moose.config.toml` to change.

A separate data analysis service lives in `apps/das/`.

## Key Files

| File | Purpose |
| --- | --- |
| `apps/brainmoose/app/datamodels/` | Data models (brainwave data) |
| `apps/brainmoose/app/functions/` | Processing functions |
| `apps/brainmoose/moose.config.toml` | Moose port and service configuration |
| `apps/das/src/` | Data analysis service source code |
| `apps/das/types/` | Type definitions for analysis |
