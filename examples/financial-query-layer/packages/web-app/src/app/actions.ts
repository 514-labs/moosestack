...
function buildUrl(
  ...
  return `${BACKEND}/transaction/metrics?${params}`;
}
...
/**
 * Execute a dynamic query against the transaction metrics API.
...
/**
 * Fetch the query model schema from the backend.
 * Returns metrics, dimensions, filters, and defaults — derived
 * directly from the transactionMetrics defineQueryModel() definition.
 */
export async function fetchSchema(): Promise<{
  ...
  const res = await fetch(`${BACKEND}/transaction/schema`, {
    cache: "no-store",
  });
  if (!res.ok) throw new Error(`Backend error: ${res.status}`);
  return res.json();
