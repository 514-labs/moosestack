...
export async function getEventsByStatus(
  startDate?: Date,
  endDate?: Date,
): Promise<StatusCountResult[]> {
  const whereClause = buildDateFilteredWhereClause(startDate, endDate);
  return await executeQuery<StatusCountResult>(
    sql`SELECT ${Events.columns.status} as status, COUNT(*) as count FROM ${Events} ${whereClause} GROUP BY ${Events.columns.status}`,
  );
}
